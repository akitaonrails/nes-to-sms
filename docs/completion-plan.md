...
about 5.4 updates/second during active traversal and frequent rebuild blanking;
...
A full-mode binary lower-bound search now accelerates the existing bank-aware
scan without changing its records or precedence. It adds 128 count bytes and
a 72-byte helper/call, with no new RAM; the same full route gains another
16.79%. Absolute blanking is unchanged, and general inlining remains research,
