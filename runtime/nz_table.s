; nz_table.s — pinned fixed-page runtime lookup tables. MUST be the LAST
; runtime include: the forced $3D00/$3E00 pages move WLA's placement cursor,
; and anything included after them would spill past the 16 KiB bank-0 boundary.

; ─── Bit-reversal lookup table ───────────────────────────────────────────────
; rt_bit_reverse_table[v] reverses the eight bits in v. Sprite conversion uses
; a fixed-page `ld l,a` / `ld h,>rt_bit_reverse_table` / `ld a,(hl)` lookup.
.orga $3d00
.section "bit_reverse_table" force
rt_bit_reverse_table:
