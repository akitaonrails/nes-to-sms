...
mod tests {
    ...
    fn mmc3_cooperative_wait_signal_preserves_runtime_abi() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        let rom = std::fs::read(path.join("sms.sms")).unwrap();
        for iff in [false, true] {
            for enable in [0, 1, 0xff] {
                for tick in [0, 1, 0x7f, 0x80, 0xff] {
                    let mut bus = SmsBus::new(rom.clone(), 0xff);
                    bus.write(0xc010, tick);
                    bus.write(0xc01c, enable);
                    bus.write(0xcb03, 0xa5);
                    ...
                    let mapping = (bus.mapper_control, bus.slot_bank);
                    let mut cpu = Cpu::new();
                    cpu.pc = defs["rt_mmc3_wait_boundary"].1;
                    cpu.sp = 0xdff0;
                    cpu.a = 0x69;
                    cpu.f = 0x95;
                    cpu.set_hl(0xc010);
                    ...
                    assert_eq!(cpu.pc, 7);
                    assert_eq!(bus.read(0xc825), u8::from(enable != 0 && tick < 0x80));
                    assert_eq!((bus.read(0xc010), bus.read(0xc01c)), (tick, enable));
                    assert_eq!(
                        (cpu.a, cpu.f, cpu.bc(), cpu.de(), cpu.hl()),
                        (0x69, 0x95, 0xc01c, 0x52a9, 0xc010)
                    );
                    assert_eq!((cpu.iff1, cpu.iff2, bus.read(0xcb03)), (iff, iff, 0xa5));
                    assert_eq!((bus.mapper_control, bus.slot_bank), mapping);
                }
            }
        }
    ...
    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT assembled MMC3_FULL_RUNTIME fixture"]
    fn mmc3_start_injection_survives_host_frames_until_guest_polls() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        let defs = load_wla_symbol_defs(&path.join("sms.sym"));
        let rom = std::fs::read(path.join("sms.sms")).unwrap();
        let call = |bus: &mut SmsBus, pc: u16, a: u8| {
            let mut cpu = Cpu::new();
            cpu.pc = pc;
            cpu.sp = 0xdff0;
            cpu.a = a;
        ...
        let latch = defs["rt_controller_latch"].1;
        let strobe = defs["rt_controller_strobe"].1;
        let read = defs["rt_controller_read"].1;
        let mut bus = SmsBus::new(rom, 0xff);
        // Invoke the actual SMS Pause vector, not a synthetic counter write.
        assert_eq!(call(&mut bus, 0x66, 0xa5), 0xa5);
        assert_eq!(bus.read(0xcb2e), 4);
        for _ in 0..150 {
            call(&mut bus, latch, 0);
        }
        assert_eq!(bus.read(0xcb2e), 4);
        assert_eq!(bus.read(0xcb06), 8);
        for poll in 0..4 {
            call(&mut bus, strobe, 0);
            let mut buttons = 0;
            for bit in 0..8 {
                assert_eq!(call(&mut bus, read, 0x17), 0);
                assert_eq!(bus.read(0xcb07), bit);
                assert_eq!(bus.read(0xcb2e), 4 - poll);
            ...
            assert_eq!(buttons, 8);
            assert_eq!(bus.read(0xcb2e), 3 - poll);
            assert_eq!(call(&mut bus, read, 0x16), 1);
            assert_eq!(bus.read(0xcb2e), 3 - poll);
            call(&mut bus, latch, 0);
        }
        assert_eq!(bus.read(0xcb06), 0, "next host latch releases Start");
        // Repeated strobes / incomplete bytes cannot consume a fresh press.
        call(&mut bus, 0x66, 0);
        for _ in 0..10 {
            call(&mut bus, strobe, 0);
            for _ in 0..7 {
                call(&mut bus, read, 0x16);
            }
            call(&mut bus, latch, 0);
        }
        assert_eq!(bus.read(0xcb2e), 4);
        assert_eq!(bus.read(0xcb06), 8);
        // Action mapping never consults SMB1's mode byte; test each button.
        bus.write(0xcb2e, 0);
        for mode in [0, 1, 0xff] {
            bus.write(0xc770, mode);
            for (sms_bit, nes_bit) in [(0, 4), (1, 5), (2, 6), (3, 7), (4, 0), (5, 1)] {
                bus.controller_port_dc = !(1 << sms_bit);
                call(&mut bus, latch, 0);
    ...
    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT assembled MMC3_FULL_RUNTIME fixture"]
    fn mmc3_full_bus_and_wide_frames_preserve_runtime_abi() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        for iff in [false, true] {
            for bank in [31, 32, 64, 95] {
                for helper in [
                    ...
                    for (ptr, frame) in [(0xd304u16, 0xd300u16), (0xd500, 0xd3f8), (0xd600, 0xd5fc)]
                    {
                        let mut bus = SmsBus::new(rom.clone(), 0xff);
                        bus.write(0xcb76, ptr as u8);
                        bus.write(0xcb77, (ptr >> 8) as u8);
                        bus.write(frame, 0xff);
                        bus.write(frame + 2, 0xc0);
                        bus.write(frame + 3, bank);
                        bus.write(0xcb02, 0xff);
                        bus.write(0xc100, 0x23);
                        bus.write(0xc101, 0x81);
                        bus.write(0xcb03, 0xa5);
                        let mut cpu = Cpu::new();
                        cpu.pc = defs[helper].1;
                        cpu.sp = 0xdff0;
                        cpu.a = 0x69;
                        cpu.f = 0x95;
                        cpu.set_bc(0x8123);
                        ...
                        assert_eq!(cpu.pc, 7, "{helper} bank={bank}");
                        assert_eq!(
                            (cpu.a, cpu.f, cpu.de(), bus.read(0xcb02), bus.read(0xcb03)),
                            (0x69, 0x95, 0x52a9, 0xff, 0xa5)
                        );
                        assert_eq!(
                            bus.read(0xcb76) as u16 | ((bus.read(0xcb77) as u16) << 8),
                            frame
                        );
            ...
            for protect in [0, 0x40, 0x80, 0xc0] {
                for control in [0, 8, 12] {
                    for writing in [false, true] {
                        let mut bus = SmsBus::new(rom.clone(), 0xff);
                        bus.cart_ram[0] = 0xab;
                        bus.cart_ram[0x4000] = 0x5a;
                        bus.write(0xfffc, control);
                        bus.write(0xfffe, 95);
                        bus.write(0xcb14, 17);
                        bus.write(0xc81e, protect);
                        bus.write(0xcb03, 0xa5);
                        let mapping = (bus.slot_bank, bus.read(0xfffc));
                        let mut cpu = Cpu::new();
                        cpu.pc = defs[if writing {
                            "rt_mmc3_write_bus"
                        } else {
                            "rt_mmc3_read_bus"
                        }]
                        ...
                        if !writing && protect & 0x80 == 0 {
                            assert_eq!(bus.read(0xcb1d), 0xe8);
                        } else {
                            assert_eq!(cpu.pc, 7);
                            assert_eq!(cpu.a, if writing { 0x6d } else { 0x5a });
                            assert_eq!((cpu.bc(), cpu.de(), cpu.hl()), (0x4587, 0x52a9, 0x6000));
                            assert_eq!((cpu.iff1, cpu.iff2), (iff, iff));
                        }
                        assert_eq!((bus.slot_bank, bus.read(0xfffc)), mapping);
                        assert_eq!(bus.cart_ram[0], 0xab);
                        assert_eq!(
                            bus.cart_ram[0x4000],
                            if writing && protect == 0x80 {
                                0x6d
                            } else {
                                0x5a
                            }
        ...
        for iff in [false, true] {
            for bank in [31u8, 32, 64, 95] {
                for owned in [false, true] {
                    for (ptr, frame) in [(0xd304u16, 0xd300u16), (0xd500, 0xd3f8), (0xd600, 0xd5fc)]
                    {
                        ...
                        bus.write(0xcb76, ptr as u8);
                        bus.write(0xcb77, (ptr >> 8) as u8);
                        bus.write(frame, 0xff); // full-frame call-time guest S
                        bus.write(frame + 1, 7);
                        bus.write(frame + 2, if owned { 0xc0 } else { 0x40 });
                        bus.write(frame + 3, bank);
                        bus.write(0xcb02, 0xff);
                        // Owned returns consume this live pair: E1FF + 1.
                        // Guest S=FF wraps both reads to $0100/$0101.
                        bus.write(0xc100, 0xff);
                        bus.write(0xc101, 0xe1);
                        bus.write(0xc81c, 7);
                        bus.write(0xcb7e, u8::from(!iff));
                        bus.write(0xcb03, 0xa5);
                        let mut cpu = Cpu::new();
                        cpu.pc = defs["rt_translated_rts"].1;
                        cpu.sp = 0xdff0;
                        cpu.a = 0x69;
                        cpu.set_de(0x52a9);
                        cpu.iff1 = iff;
                        ...
                        for _ in 0..2000 {
                            if cpu.pc == destination.1 || bus.read(0xcb1d) != 0 {
                                break;
                            }
                            cpu.step(&mut bus).unwrap();
                        }
                        assert_eq!(cpu.pc, destination.1, "bank={bank} owned={owned}");
                        assert_eq!(bus.read(0xcb14), destination.0);
                        assert_eq!(bus.slot_bank[1], destination.0);
                        assert_eq!(bus.read(0xcb02), if owned { 1 } else { 0xff });
                        assert_eq!(
                            bus.read(0xcb76) as u16 | ((bus.read(0xcb77) as u16) << 8),
                            frame
                        );
                        assert_eq!(
                            (cpu.a, cpu.de(), cpu.sp, bus.read(0xcb03)),
                            (0x69, 0x52a9, 0xdff0, 0xa5)
                        );
                        assert_eq!((cpu.iff1, cpu.iff2), (iff, iff));
                    }
                }
            ...
            for control in [0, 8, 12] {
                for address in [0x0000u16, 0x1fff, 0x6000, 0x7fff, 0x8000, 0xffff] {
                    let mut bus = SmsBus::new(rom.clone(), 0xff);
                    bus.write(0xfffe, 95);
                    bus.write(0xcb14, 17); // nested data mapping differs from code shadow
                    bus.write(0xffff, 104);
                    bus.write(0xfffc, control);
                    bus.write(0xc81e, 0x80);
                    bus.write(0xcb03, 0xa5);
                    ...
                    let mapping = (bus.slot_bank, bus.read(0xfffc));
                    let mut cpu = Cpu::new();
                    cpu.pc = defs["rt_mmc3_read_bus"].1;
                    cpu.sp = 0xdff0;
                    cpu.set_bc(0x4587);
                    cpu.set_de(0x52a9);
                    cpu.set_hl(address);
                    ...
                    assert_eq!(cpu.pc, 7, "read ${address:04X}");
                    assert_eq!((cpu.bc(), cpu.de(), cpu.hl()), (0x4587, 0x52a9, address));
                    assert_eq!((bus.slot_bank, bus.read(0xfffc)), mapping);
                    assert_eq!(bus.read(0xcb03), 0xa5);
                    assert_eq!((cpu.iff1, cpu.iff2, cpu.ei_pending), (iff, iff, 0));
                }
    ...
    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT assembled MMC3_FULL_RUNTIME fixture"]
    fn mmc3_full_ppu_preserves_bus_state_and_physical_chr_semantics() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        let chr_base = define("MMC3_CHR_DATA_BASE") * BANK_SIZE;
        let pages = define("MMC3_CHR_BANK_COUNT");
        let mut rom = std::fs::read(path.join("sms.sms")).unwrap();
        // Synthetic physical source markers, not injected game state or a
        // claimed playable-ROM run. Execute the actual assembled helpers.
        for page in 0..pages {
            for offset in 0..0x400 {
                rom[chr_base + page * 0x400 + offset] =
                    (page as u8).wrapping_mul(29).wrapping_add(offset as u8);
            }
        }
        for iff in [false, true] {
            for control in [0, 8, 12] {
                for inversion in [0, 0x80] {
                    let mut bus = SmsBus::new(rom.clone(), 0xff);
                    bus.write(0xfffe, 17);
                    bus.write(0xcb14, 17);
                    bus.write(0xffff, 99);
                    ...
                    bus.write(0xcb03, 0xa5);
                    let invoke =
                        |bus: &mut SmsBus, label: &str, value: u8, reg: u8, address: u16| -> u8 {
                            assert_eq!(defs[label].0, 0);
                            let mapping = (bus.slot_bank, bus.read(0xfffc));
                            let mut cpu = Cpu::new();
                            cpu.pc = defs[label].1;
                            cpu.sp = 0xdff0;
                            ...
                            assert_eq!(cpu.sp, 0xdff2);
                            assert_eq!(cpu.de(), 0x52a9, "{label} changed DE");
                            assert_eq!(bus.read(0xcb03), 0xa5);
                            assert_eq!(bus.read(0xcb14), 17);
                            assert_eq!(
                                (bus.slot_bank, bus.read(0xfffc)),
                                mapping,
                                "{label} leaked mapping"
                            );
                            assert_eq!((cpu.iff1, cpu.iff2, cpu.ei_pending), (iff, iff, 0));
                            if label == "rt_ppu_write" || label == "rt_mmc3_write" {
                                assert_eq!((cpu.a, cpu.f), (value, 0x95));
                            }
                            cpu.a
                        };
                    let registers = [0xff, 0xfc, 0x83, 0x9a, 5, 0x26];
                    for (register, value) in registers.into_iter().enumerate() {
                        invoke(
                            &mut bus,
                            "rt_mmc3_write",
                            register as u8 | inversion,
                            0,
                            0x8000,
                        );
                        invoke(&mut bus, "rt_mmc3_write", value, 0, 0x8001);
                    }
                    let mask = (pages - 1) as u8;
                    let mut map = [
                        registers[0] & !1,
                        registers[0] | 1,
                        registers[1] & !1,
                    ...
                    for (index, page) in map.into_iter().enumerate() {
                        assert_eq!(bus.read(0xc850 + index as u16), page);
                        for offset in [0, 0x3ffu16] {
                            let address = (index as u16) * 0x400 + offset;
                            bus.write(0xcb0b, 0);
                            invoke(&mut bus, "rt_ppu_write", (address >> 8) as u8, 6, 0);
                            invoke(&mut bus, "rt_ppu_write", address as u8, 6, 0);
                            bus.write(0xcb11, 0xa7);
                            assert_eq!(invoke(&mut bus, "rt_ppu_read", 0, 7, 0), 0xa7);
                            let expected = (page as u8).wrapping_mul(29).wrapping_add(offset as u8);
                            assert_eq!(bus.read(0xcb11), expected, "CHR ${address:04X}");
                            bus.write(0xcb0f, (address >> 8) as u8);
                            bus.write(0xcb10, address as u8);
                            invoke(&mut bus, "rt_ppu_write", expected ^ 0xff, 7, 0);
                            assert_eq!(
                                bus.rom[chr_base + usize::from(page) * 0x400 + usize::from(offset)],
                                expected
                            );
                        }
                    }
                    for mirroring in [0, 1] {
                        invoke(&mut bus, "rt_mmc3_write", mirroring, 0, 0xa000);
                        for page in 0..4u16 {
                            bus.write(0xcb0f, 0x20 + (page * 4) as u8);
                            bus.write(0xcb10, 0x31);
                            invoke(&mut bus, "rt_ppu_write", 0x60 + page as u8, 7, 0);
                            let ciram_page = if mirroring == 0 { page & 1 } else { page >> 1 };
                            let old = bus.read(0xfffc);
                            bus.write(0xfffc, 8);
                            assert_eq!(bus.read(0x8031 + ciram_page * 0x400), 0x60 + page as u8);
                            bus.write(0xfffc, old);
                            bus.write(0xcb0f, 0x30 + (page * 4) as u8);
                            bus.write(0xcb10, 0x31);
                            invoke(&mut bus, "rt_ppu_read", 0, 7, 0);
                            assert_eq!(bus.read(0xcb11), 0x60 + page as u8);
                        }
                    }
                    // Palette aliases are immediate reads, and refill the
                    // delayed read buffer from the mirrored nametable below.
                    for index in [0x10u8, 0x14, 0x18, 0x1c] {
                        bus.write(0xcb0f, 0x3f);
                        bus.write(0xcb10, index);
                        invoke(&mut bus, "rt_ppu_write", 0x2b, 7, 0);
                        assert_eq!(bus.read(0xc860 + u16::from(index & 0x0f)), 0x2b);
                        bus.write(0xcb0f, 0x3f);
                        bus.write(0xcb10, index & 0x0f);
                        assert_eq!(invoke(&mut bus, "rt_ppu_read", 0, 7, 0), 0x2b);
                    }
                    invoke(&mut bus, "rt_ppu_write", 4, 0, 0);
                    bus.write(0xcb0f, 0x3f);
                    bus.write(0xcb10, 0xf0);
                    invoke(&mut bus, "rt_ppu_write", 0x13, 7, 0);
                    assert_eq!((bus.read(0xcb0f), bus.read(0xcb10)), (0x40, 0x10));
                    // Shared loopy w: a status read resets both aliases;
                    // scroll0/239 produces the native SMB3 TADD=$73A0.
                    invoke(&mut bus, "rt_ppu_read", 0, 2, 0);
                    invoke(&mut bus, "rt_ppu_write", 0xa8, 0, 0);
                    invoke(&mut bus, "rt_ppu_write", 0, 5, 0);
                    invoke(&mut bus, "rt_ppu_write", 239, 5, 0);
                    assert_eq!((bus.read(0xc834), bus.read(0xc835)), (0x73, 0xa0));
                    assert_eq!((bus.read(0xcb0b), bus.read(0xcb0e)), (0, 0));
                    // Actual IRQ intent distinguishes $2006 reload from
                    // temporary scroll latches. Match the selected native
                    // reference's visible $2007 read vertical increment.
                    for active in [0, 1] {
                        for mask in [0, 0x18] {
                            bus.write(0xc8f7, active);
                            invoke(&mut bus, "rt_ppu_write", mask, 1, 0);
                            invoke(&mut bus, "rt_ppu_read", 0, 2, 0);
                            invoke(&mut bus, "rt_ppu_write", 0x0b, 6, 0);
                            invoke(&mut bus, "rt_ppu_write", 0, 6, 0);
                            assert_eq!(bus.read(0xc8f7), if active == 1 { 3 } else { 0 });
                            for (before, after) in [
                                (0x0b00u16, 0x1b00),
                                (0x6ba0, 0x7ba0),
                                (0x7ba0, 0x0000),
                                ...
                                bus.write(0xcb0f, (before >> 8) as u8);
                                bus.write(0xcb10, before as u8);
                                invoke(&mut bus, "rt_ppu_read", 0, 7, 0);
                                let expected = if active == 1 && mask != 0 {
                                    after
                                } else {
                                    before + 1
                                };
                                assert_eq!(
                                    u16::from_be_bytes([bus.read(0xcb0f), bus.read(0xcb10)]),
                                    expected
                                );
                            }
                        }
    ...
    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT assembled MMC3_FULL_RUNTIME fixture"]
    fn mmc3_full_renderer_uses_frozen_chr_and_exact_fine_y_split() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        let chr_base: usize = asm
            ...
            * BANK_SIZE;
        let mut rom = std::fs::read(path.join("sms.sms")).unwrap();
        // Two physically distinct, deliberately simple planar patterns.
        // The same logical NES tile is selected on both sides of the IRQ.
        for (page, planes) in [(0, [0xaa, 0x55]), (2, [0xf0, 0xcc])] {
            rom[chr_base + page * 0x400..chr_base + page * 0x400 + 8].fill(planes[0]);
            rom[chr_base + page * 0x400 + 8..chr_base + page * 0x400 + 16].fill(planes[1]);
        }
        for row in 0..16usize {
            let tile = row / 8;
            rom[chr_base + 4 * 0x400 + tile * 16 + row % 8] = 1 << (row % 8);
            rom[chr_base + 4 * 0x400 + tile * 16 + row % 8 + 8] = if row < 8 { 0x55 } else { 0xaa };
        }
        for fine_y in [0u8, 7] {
            for split in [192u8, 193] {
                let mut bus = SmsBus::new(rom.clone(), 0xff);
                bus.write(0xfffe, 17);
                bus.write(0xcb14, 17);
                bus.write(0xfffc, 8);
                bus.write(0xc831, 2); // renderer owns a frozen packet
                bus.write(0xcb08, 0xa8);
                bus.write(0xcb09, 0x18);
                bus.write(0xc824, 1); // horizontal CIRAM mirroring
                bus.write(0xc834, if fine_y == 7 { 0x73 } else { 0 });
                bus.write(0xc835, if fine_y == 7 { 0xa0 } else { 0 });
                for offset in 0..0x800 {
                    bus.write(0x8000 + offset, 0);
                }
                for offset in 0..0x100 {
                    bus.write(0xc900 + offset, 0xe0);
                }
                let sprite_attr =
                    3 | if fine_y == 7 { 0x40 } else { 0 } | if split == 193 { 0x80 } else { 0 };
                for (offset, value) in [24, 1, sprite_attr, 17].into_iter().enumerate() {
                ...
                let invoke = |bus: &mut SmsBus, label: &str| {
                    ...
                    let mut di_start = 0;
                    let mut max_di = 0;
                    let mut di_pc = cpu.pc;
                    let mut max_di_pc = (cpu.pc, cpu.pc);
                    for _ in 0..3_000_000 {
                        if cpu.pc == 7 {
                            break;
                        }
                        let enabled = cpu.iff1;
                        cpu.step(bus).unwrap();
                        if !enabled && cpu.iff1 && cpu.cycles - di_start > max_di {
                            max_di = cpu.cycles - di_start;
                            max_di_pc = (di_pc, cpu.pc);
                        }
                        if enabled && !cpu.iff1 {
                            di_start = cpu.cycles;
                            di_pc = cpu.pc;
                    ...
                    assert!(
                        max_di < 59_736,
                        "{label} masked host interrupts for {max_di}T at {max_di_pc:04X?}"
                    );
                    (cpu.cycles, max_di)
                };
                ...
                bus.write(0xc850, 2);
                for index in 0..32 {
                    bus.write(0xc860 + index, 0x20 + index as u8);
                }
                invoke(&mut bus, "rt_mmc3_capture_hud");
                // Later live producer changes must not affect this frame.
                bus.write(0xc850, 4);
                bus.write(0x8000, 0x7f);
                bus.write(0xc837, split);
                let (cycles, max_di) = invoke(&mut bus, "rt_mmc3_frame_render");
                eprintln!(
                    "MMC3 renderer fixture fineY={fine_y} split={split}: interpreterT={cycles}, maxDI={max_di}T (no host IRQ delivery in this helper test)"
                );
                for y in 0..224usize {
                    for x in 0..256usize {
                        let source_y = y + usize::from(fine_y);
                        let entry = 0x3700 + (source_y / 8 * 32 + x / 8) * 2;
                        let tile = usize::from(bus.vram[entry])
                            | (usize::from(bus.vram[entry + 1] & 1) << 8);
                        let pattern = tile * 32 + (source_y & 7) * 4;
                        let pixel = (0..4).fold(0u8, |value, plane| {
                            value | (((bus.vram[pattern + plane] >> (7 - x % 8)) & 1) << plane)
                        });
                        let planes = if y < usize::from(split) {
                            [0xaa, 0x55]
                        } else {
                            [0xf0, 0xcc]
                        };
                        let expected = ((planes[0] >> (7 - x % 8)) & 1)
                            | (((planes[1] >> (7 - x % 8)) & 1) << 1);
                        assert_eq!(pixel, expected, "fineY={fine_y} split={split} at{x},{y}");
                    }
                }
                for index in 0..32usize {
                    let source = if index % 4 == 0 { 0 } else { index as u8 };
                    assert_eq!(bus.cram[index], assets::nes_palette_to_sms_color(source));
                }
                assert_eq!(bus.vram[0x3f00], 24);
                assert_eq!(&bus.vram[0x3f80..0x3f82], &[17, 0]);
                for row in 0..16usize {
                    let source_row = if sprite_attr & 0x80 != 0 {
                        15 - row
                    } else {
                        row
                    };
                    let mut p0 = 1u8 << (source_row % 8);
                    let mut p1: u8 = if source_row < 8 { 0x55 } else { 0xaa };
                    if sprite_attr & 0x40 != 0 {
                        p0 = p0.reverse_bits();
                        p1 = p1.reverse_bits();
                    }
                    assert_eq!(
                        &bus.vram[0x2000 + row * 4..0x2004 + row * 4],
                        &[p0, p1, p0 | p1, p0 | p1],
                        "8x16 attr={sprite_attr:02X} row{row}"
                    );
                }
                bus.write(0xc831, 0);
    ...
    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT assembled MMC3_FULL_RUNTIME fixture"]
    fn mmc3_full_renderer_respects_irq_vertical_reload() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        for tile in 0..64usize {
            for row in 0..8 {
                let value = (tile * 8 + row) as u8;
                rom[chr_base + tile * 16 + row] = value;
                rom[chr_base + tile * 16 + row + 8] = !value;
            }
        }
        for fine_y in [0u8, 7] {
            for split in [192u8, 193] {
                for irq_fine_y in [0u8, 1] {
                    let mut bus = SmsBus::new(rom.clone(), 0xff);
                    bus.write(0xfffc, 8);
                    bus.write(0xc831, 1);
                    bus.write(0xcb08, 0xa8);
                    bus.write(0xcb09, 0x18);
                    bus.write(0xc824, 1);
                    // Different nametable rows and planes expose both wrong
                    // coarse origin and a one-pixel fine-Y composition error.
                    for page in 0..2u16 {
                        for row in 0..30u16 {
                            for col in 0..32 {
                                bus.write(
                                    0x8000 + page * 0x400 + row * 32 + col,
                                    (page * 32 + row) as u8,
                                );
                            }
                        }
                    ...
                    bus.write(0xc834, 3 | (fine_y << 4));
                    bus.write(0xc835, 0x40); // NMI origin coarseY26, NT2000
                    let invoke = |bus: &mut SmsBus, label: &str| {
                        let mut cpu = Cpu::new();
                        cpu.pc = defs[label].1;
                        cpu.sp = 0xdff0;
                    ...
                    invoke(&mut bus, "rt_mmc3_capture_playfield");
                    bus.write(0xc8f7, 3); // actual IRQ + explicit $2006 reload
                    bus.write(0xcb0f, 0x0b | (irq_fine_y << 4));
                    bus.write(0xcb10, 0);
                    bus.write(0xc834, 0x73);
                    bus.write(0xc835, 0xa0); // final t differs from live v
                    invoke(&mut bus, "rt_mmc3_capture_hud");
                    assert_eq!(
                        (bus.read(0x9970), bus.read(0x9971), bus.read(0x9972)),
                        (0, 0x0b | (irq_fine_y << 4), 1)
                    );
                    bus.write(0xc8f7, 0);
                    bus.write(0xc837, split);
                    invoke(&mut bus, "rt_mmc3_frame_render");
                    for y in 0..224usize {
                        let (origin, delta) = if y < usize::from(split) {
                            (26 * 8 + usize::from(fine_y), y)
                        } else {
                            (
                                240 + 24 * 8 + usize::from(irq_fine_y),
                                y - usize::from(split),
                            )
                        };
                        let source_y = (origin + delta) % 480;
                        let tile = (source_y / 240) * 32 + (source_y % 240) / 8;
                        let p0 = (tile * 8 + source_y % 8) as u8;
                        let grid_y = y + usize::from(fine_y);
                        for x in 0..256usize {
                            let entry = 0x3700 + (grid_y / 8 * 32 + x / 8) * 2;
                            let slot = usize::from(bus.vram[entry])
                                | (usize::from(bus.vram[entry + 1] & 1) << 8);
                            let row = slot * 32 + grid_y % 8 * 4;
                            let pixel = ((bus.vram[row] >> (7 - x % 8)) & 1)
                                | (((bus.vram[row + 1] >> (7 - x % 8)) & 1) << 1);
                            let expected =
                                ((p0 >> (7 - x % 8)) & 1) | (((!p0 >> (7 - x % 8)) & 1) << 1);
                            assert_eq!(
                                pixel, expected,
                                "NMI fineY{fine_y} split{split} IRQ fineY{irq_fine_y} at{x},{y}"
                            );
                        }
                    }
                }
        ...
        for (fine_y, split, table, inverted) in [
            ...
            for (addresses, records, bg_changed) in cases {
                ...
                for mode in 0..3 {
                    ...
                    let mut bus = SmsBus::new(rom.clone(), 0xff);
                    bus.ram[0x1d40..0x1d60].fill(0xff); // initial source fixture
                    bus.write(0xfffc, 8);
                    bus.write(0xc831, 1);
                    ...
                    let write_source_or_state = |bus: &mut SmsBus, address, value| {
                        if (0x8000..0x8800).contains(&address) {
                            let offset = address - 0x8000;
                            let logical = if bus.read(0xc824) != 0 && offset >= 0x400 {
                                0x2800 + (offset & 0x3ff)
                            } else {
                                0x2000 + offset
                            };
                            let v = (bus.read(0xcb0f), bus.read(0xcb10));
                            mmc3_test_ciram_store(bus, defs["rt_ppu_write"].1, logical, value);
                            // This synthetic dependency case varies only the
                            // physical source byte, not the separate IRQ-v input.
                            bus.write(0xcb0f, v.0);
                            bus.write(0xcb10, v.1);
                        } else {
                    ...
                    let capture = |bus: &mut SmsBus, mutate: bool| {
                        ...
                        if mutate && records == 1 {
                            for &(address, value) in &original {
                                write_source_or_state(bus, address, value);
                            }
                        }
                        bus.write(0xc8f7, 3);
                        ...
                        if mutate && records == 2 {
                            for &(address, _) in &original {
                                ...
                                write_source_or_state(
                                    bus,
                                    address,
                                    value ^ if address == 0xc8f7 { 2 } else { 1 },
                                );
                            }
                        }
    ...
    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT assembled MMC3_FULL_RUNTIME fixture"]
    fn mmc3_full_packets_reuse_only_exact_committed_state() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        let capture = |bus: &mut SmsBus| {
            invoke(bus, "rt_mmc3_capture_playfield") + invoke(bus, "rt_mmc3_capture_hud")
        };
        let first_capture = capture(&mut bus);
        assert_eq!(bus.read(0xc8f4), 1);
        let rebuild = invoke(&mut bus, "rt_mmc3_frame_present");
        assert_eq!(bus.read(0xc833), 1);
        let vram = bus.vram;
        let cram = bus.cram;
        let registers = bus.vdp_regs;
        bus.write(0xc831, 1);
        let same_capture = capture(&mut bus);
        assert_eq!(bus.read(0xc8f4), 0);
        let reuse = invoke(&mut bus, "rt_mmc3_frame_present");
        assert_eq!(bus.read(0xc831), 1, "reuse must not enter renderer");
        assert_eq!(bus.vram, vram);
        assert_eq!(bus.cram, cram);
        assert_eq!(bus.vdp_regs, registers);
        assert!(reuse < 500); // exact reuse plus mandatory layer validation
        eprintln!(
            "MMC3 exact-packet synthetic interpreterT: firstCapture={first_capture} rebuild={rebuild}, unchangedCapture={same_capture} reuse={reuse}"
        );

        // Every source identity participates, including bytes the present
        // viewport does not currently display. No hash is a correctness gate.
        for address in [
            0x8000, 0x87ff, 0xc900, 0xc9ff, 0xc850, 0xc857, 0xc860, 0xc87f, 0xcb08, 0xcb09, 0xcb0c,
            0xcb0d, 0xc834, 0xc835, 0xc836, 0xc824,
        ] {
            capture(&mut bus);
            assert_eq!(bus.read(0xc8f4), 0);
            invoke(&mut bus, "rt_mmc3_frame_present"); // complete exact packet before next producer
            let old = bus.read(address);
            bus.write(address, old ^ 1);
            capture(&mut bus);
            assert_eq!(bus.read(0xc8f4), 1, "identity {address:04X} ignored");
            bus.write(address, old);
            capture(&mut bus);
            invoke(&mut bus, "rt_mmc3_frame_present"); // restored source is committed, not merely captured
        }
        capture(&mut bus);
        // A HUD-only producer write must be observed after playfield freeze.
        bus.write(0x87fe, 1);
        invoke(&mut bus, "rt_mmc3_capture_hud");
        assert_eq!(bus.read(0xc8f4), 1);
        bus.write(0x87fe, 0);
        ...
        capture(&mut bus);
        invoke(&mut bus, "rt_mmc3_frame_present");
        for (before, after) in [(0x1b00u16, 0x1b01u16), (0x1b00, 0x2b00)] {
            bus.write(0xc8f7, 3);
            bus.write(0xcb0f, (before >> 8) as u8);
            bus.write(0xcb10, before as u8);
            invoke(&mut bus, "rt_mmc3_capture_hud");
            ...
            assert_eq!(bus.read(0xc8f4), 0);
            bus.write(0xc8f7, 3);
            bus.write(0xcb0f, (after >> 8) as u8);
            bus.write(0xcb10, after as u8);
            invoke(&mut bus, "rt_mmc3_capture_hud");
            assert_eq!(bus.read(0xc8f4), 1, "IRQ live-v identity must participate");
            invoke(&mut bus, "rt_mmc3_frame_present");
        }
        bus.write(0xc8f7, 0);
        invoke(&mut bus, "rt_mmc3_capture_playfield");
        ...
        assert_eq!(
            bus.read(0xc8f4),
            1,
            "removing IRQ reload intent must participate"
        );
        capture(&mut bus);
        bus.write(0xc837, 192);
        assert!(invoke(&mut bus, "rt_mmc3_frame_present") > 100_000);
        assert_eq!(bus.read(0xc8f0), 192, "split-only changes must publish");
        bus.write(0xcb09, 0);
        capture(&mut bus);
        invoke(&mut bus, "rt_mmc3_frame_present");
        assert_eq!(bus.read(0xc833), 0);
        assert_eq!(
            bus.vdp_regs[1] & 0x40,
            0,
            "completed render-off packet must blank"
        );
    }

    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT assembled MMC3_FULL_RUNTIME fixture"]
    fn mmc3_full_partial_layers_trap_before_publication_or_reuse() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        let defs = load_wla_symbol_defs(&path.join("sms.sym"));
        let rom = std::fs::read(path.join("sms.sms")).unwrap();
        let invoke = |bus: &mut SmsBus, entry: &str| {
            let mut cpu = Cpu::new();
            cpu.pc = defs[entry].1;
            cpu.sp = 0xdff0;
            bus.write(cpu.sp, 7);
            ...
            assert!(cpu.pc == 7 || bus.read(0xcb1d) != 0);
        };
        for entry in ["rt_mmc3_frame_present", "rt_mmc3_frame_render"] {
            for ready in [0, 1] {
                for changed in [0, 1] {
                    for record in [0x9900, 0x9940] {
                        for partial in [0x08, 0x10] {
                            let mut bus = SmsBus::new(rom.clone(), 0xff);
                            bus.write(0xfffc, 8);
                            bus.write(0x9909, 0x18);
                            bus.write(0x9949, 0x18);
                            bus.write(record + 9, partial);
                            bus.write(0xc833, ready);
                            bus.write(0xc8f4, changed);
                            bus.vram.fill(0x5a);
                            bus.cram.fill(0x21);
                            bus.vdp_regs[1] = if ready == 1 { 0xf2 } else { 0xb2 };
                            bus.vdp_regs[7] = 0x0d;
                            let video = (bus.vram, bus.cram, bus.vdp_regs);
                            bus.write(0xfffc, 0); // entry must select the frozen SRAM records
                            invoke(&mut bus, entry);
                            assert_eq!(
                                bus.read(0xcb1d),
                                0xea,
                                "{entry} accepted record{record:04X} mask{partial:02X}"
                            );
                            assert_eq!(
                                (bus.vram, bus.cram, bus.vdp_regs),
                                video,
                                "unsupported layers must not blank, repaint, recolor, or publish a frame"
                            );
                        }
                    }
                }
        ...
        for mask in [0x00, 0x18, 0x1e] {
            let mut bus = SmsBus::new(rom.clone(), 0xff);
            bus.write(0xfffc, 8);
            for record in [0x9900, 0x9940] {
                bus.write(record + 8, 0xa8);
            ...
            invoke(&mut bus, "rt_mmc3_frame_present");
            assert_eq!(bus.read(0xcb1d), 0);
            assert_eq!(bus.read(0xc833), u8::from(mask != 0));
            assert_eq!(bus.vdp_regs[1] & 0x40, if mask == 0 { 0 } else { 0x40 });
            if mask != 0 {
                assert_eq!(bus.vdp_regs[0] & 0x20, if mask == 0x18 { 0x20 } else { 0 });
            }
        }
    }
    ...
    fn mmc3_test_ciram_store(bus: &mut SmsBus, entry: u16, address: u16, value: u8) {
        let mapping = (bus.slot_bank, bus.read(0xfffc));
        bus.write(0xcb0f, (address >> 8) as u8);
        bus.write(0xcb10, address as u8);
        let mut cpu = Cpu::new();
        ...
        assert_eq!((cpu.pc, cpu.sp), (7, 0xdff2));
        assert_eq!((bus.slot_bank, bus.read(0xfffc)), mapping);
    }

    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT assembled MMC3_FULL_RUNTIME fixture"]
    fn mmc3_full_ciram_touches_follow_physical_stores() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        bus.write(0xfffc, 12);
        bus.write(0xcb03, 0xa5);
        bus.ram[0x1300..0x1600].fill(0x76);
        bus.cart_ram[0x4000..0x6000].fill(0x96);
        let invoke = |bus: &mut SmsBus, label: &str, reg: u8, value: u8, iff: bool| {
            let mapping = (bus.slot_bank, bus.read(0xfffc));
            let mut cpu = Cpu::new();
            cpu.pc = defs[label].1;
            cpu.sp = 0xdff0;
            ...
            assert_eq!((cpu.pc, cpu.sp), (7, 0xdff2));
            if label == "rt_ppu_write" {
                assert_eq!((cpu.a, cpu.f, cpu.de()), (value, 0x95, 0x52a9));
                assert_eq!((cpu.iff1, cpu.iff2, cpu.ei_pending), (iff, iff, 0));
                assert_eq!((bus.slot_bank, bus.read(0xfffc)), mapping);
            }
            assert_eq!(bus.read(0xcb03), 0xa5);
        };
        let store = |bus: &mut SmsBus, address: u16, value: u8, iff| {
            // Seed only source-address registers, never the marking state.
            bus.write(0xcb0f, (address >> 8) as u8);
            bus.write(0xcb10, address as u8);
            invoke(bus, "rt_ppu_write", 7, value, iff);
        };
        let physical = |address: u16, horizontal: bool| -> usize {
            let address = address & 0x3fff;
            if horizontal {
                usize::from((address & 0x3ff) | ((address & 0x800) >> 1))
            } else {
                usize::from(address & 0x7ff)
            }
        };
        let mut vectors = 0;
        for horizontal in [false, true] {
            bus.write(0xc824, u8::from(horizontal));
            for offset in 0..2048u16 {
                let address = if horizontal && offset >= 1024 {
                    0x2800 + offset - 1024
                } else {
                    0x2000 + offset
                };
                bus.ram[0x1d40..0x1d60].fill(0);
                let increment = if offset & 1 == 0 { 1 } else { 32 };
                bus.write(0xcb08, if increment == 1 { 0 } else { 4 });
                let value = (offset as u8).wrapping_mul(29);
                store(&mut bus, address, value, offset & 2 != 0);
                let block = usize::from(offset / 16);
                let mut expected = [0; 16];
                expected[block / 8] = 1 << (block & 7);
                assert_eq!(&bus.ram[0x1d40..0x1d50], &expected);
                assert_eq!(&bus.ram[0x1d50..0x1d60], &expected);
                assert_eq!(bus.cart_ram[usize::from(offset)], value);
                let next = u16::from_be_bytes([bus.read(0xcb0f), bus.read(0xcb10)]);
                assert_eq!(next, address + increment);
                // Same value and change-then-restore remain conservative touches.
                for repeated in [value, value ^ 0xff, value] {
                    store(&mut bus, address, repeated, false);
                    assert_eq!(&bus.ram[0x1d40..0x1d50], &expected);
                    assert_eq!(&bus.ram[0x1d50..0x1d60], &expected);
                }
                vectors += 1;
            ...
            for address in [
                0x2000, 0x23ff, 0x2400, 0x27ff, 0x2800, 0x2bff, 0x2c00, 0x2fff, 0x3000, 0x33ff,
                0x3400, 0x37ff, 0x3800, 0x3eff,
            ] {
                bus.ram[0x1d40..0x1d60].fill(0);
                store(&mut bus, address, 0x5a, true);
                let index = physical(address, horizontal);
                let mut expected = [0; 16];
                expected[index / 128] = 1 << ((index / 16) & 7);
                assert_eq!(&bus.ram[0x1d40..0x1d50], &expected);
                assert_eq!(&bus.ram[0x1d50..0x1d60], &expected);
                assert_eq!(bus.cart_ram[index], 0x5a);
                vectors += 1;
            }
        }
        for address in [0, 0x1fff, 0x3f00, 0x3f10, 0x3fff] {
            bus.ram[0x1d40..0x1d60].fill(0x69);
            store(&mut bus, address, 0xb7, true);
            assert!(bus.ram[0x1d40..0x1d60].iter().all(|&v| v == 0x69));
        }
        invoke(&mut bus, "rt_ppu_write", 4, 0x37, true);
        assert!(bus.ram[0x1d40..0x1d60].iter().all(|&v| v == 0x69));
        for poison in [0, 0xa5] {
            bus.ram[0x1d40..0x1d60].fill(poison);
            bus.cart_ram[..0x4000].fill(0x98);
            invoke(&mut bus, "rt_mmc3_graphics_init", 0, 0, false);
            assert!(bus.cart_ram[..0x4000].iter().all(|&v| v == 0));
            assert!(bus.ram[0x1d40..0x1d60].iter().all(|&v| v == 0xff));
        }
        assert!(bus.ram[0x1300..0x1600].iter().all(|&v| v == 0x76));
        assert!(bus.cart_ram[0x4000..0x6000].iter().all(|&v| v == 0x96));
        eprintln!(
            "physical CIRAM marking:{vectors} address/mirror vectors, repeated/restored writes, negatives and two init states"
        );
    }

    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT assembled touched-CIRAM runtime"]
    fn mmc3_full_ciram_mark_bounds_whole_ppu_and_interrupts() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        let rom = std::fs::read(path.join("sms.sms")).unwrap();
        let mut maximum = 0;
        let mut closed_maximum = 0;
        let mut min_sp = 0xdff0;
        let mut injected = 0;
        for (address, increment, horizontal) in [
            (0x2000u16, 1, false),
            (0x207f, 32, false),
            (0x2fff, 1, false),
            (0x3eff, 32, true),
            ...
            for iff in [false, true] {
                ...
                while index < runs.len() {
                    let injection: Option<(u16, bool)> = runs[index];
                    let mut bus = SmsBus::new(rom.clone(), 0xff);
                    bus.write(0xfffc, 12);
                    bus.write(0xfffe, 6);
                    bus.write(0xffff, 9);
                    ...
                    bus.write(0xcb28, 1);
                    bus.write(0xcb03, 0xa5);
                    bus.write(0xc831, 1); // host service cannot enter guest code
                    bus.write(0xd9f6, 2);
                    bus.write(0xcb08, if increment == 1 { 0 } else { 4 });
                    bus.write(0xc824, u8::from(horizontal));
                    bus.write(0xcb0f, (address >> 8) as u8);
                    ...
                    let mut interrupted: Option<[u16; 6]> = None;
                    let mut fired = false;
                    let mut total = 0;
                    let mut closed = 0;
                    ...
                    for _ in 0..10_000 {
                        ...
                        if let Some([pc, sp, af, bc, de, hl]) = interrupted
                            && cpu.pc == pc
                            && cpu.sp == sp
                        {
                            assert_eq!(
                                ...
                                (af, bc, de, hl),
                                "interrupt context restored before resumed source instruction"
                            );
                            interrupted = None;
                        }
                        if interrupted.is_none() {
                            if index == 0 {
                                points.insert((cpu.pc, false));
                                if cpu.iff1 && cpu.ei_pending == 0 {
                                    points.insert((cpu.pc, true));
                                }
                            }
                            if !fired && injection.is_some_and(|(pc, _)| pc == cpu.pc) {
                                let host = injection.unwrap().1;
                                if host {
                                ...
                                cpu.iff2 = cpu.iff1;
                                cpu.iff1 = false;
                                cpu.pc = if host { 0x38 } else { 0x66 };
                                bus.frame_int_pending = host;
                                fired = true;
                                injected += 1;
                            }
                        }
                        if index == 0 {
                            let t = mmc3_row_nominal(&cpu, &mut bus);
                            total += t;
                            if !cpu.iff1 || cpu.ei_pending != 0 || bus.read(cpu.pc) == 0xf3 {
                                closed += t;
                            }
                            if cpu.pc == defs["rt_mmc3_ciram_touched"].1 {
                                marking = true;
                            }
                            if marking {
                                mark_t += t;
                                if bus.read(cpu.pc) == 0xc9 {
                                    marking = false;
                                }
                            }
                        }
                    ...
                    assert_eq!(
                        (cpu.pc, cpu.sp, cpu.a, cpu.f, cpu.de()),
                        (7, 0xdff2, 0x5a, 0x95, 0x52a9)
                    );
                    assert_eq!((cpu.iff1, cpu.iff2, cpu.ei_pending), (iff, iff, 0));
                    assert_eq!(
                        (bus.slot_bank[1], bus.slot_bank[2], bus.read(0xfffc)),
                        (6, 9, 12)
                    );
                    assert_eq!(bus.read(0xcb03), 0xa5);
                    assert_eq!(bus.read(0xc8f9), 0, "no guest event admitted");
                    assert!(bus.ram[0x1300..0x1600].iter().all(|&v| v == 0x76));
                    assert!(bus.cart_ram[0x4000..0x6000].iter().all(|&v| v == 0x96));
                    let physical = if horizontal {
                        (address & 0x3ff) | ((address & 0x800) >> 1)
                    } else {
                        address & 0x7ff
                    } as usize;
                    assert_eq!(bus.cart_ram[physical], 0x5a);
                    let mut expected = [0; 16];
                    expected[physical / 128] = 1 << ((physical / 16) & 7);
                    assert_eq!(&bus.ram[0x1d40..0x1d50], &expected);
                    assert_eq!(&bus.ram[0x1d50..0x1d60], &expected);
                    if index == 0 {
                        assert_eq!(mark_t, 151, "entire marking helper including RET");
                        maximum = maximum.max(total);
                        if iff {
                            closed_maximum = closed_maximum.max(closed);
                        }
                        runs.extend(points.into_iter().map(Some));
                    } else {
                        assert!(fired);
                    }
                    index += 1;
        ...
        // A Pause adds11T entry +55T fixed handler. Physical timing/phase
        // acceptance still belongs to the actual stock-core observer.
        assert!(maximum + 66 <= 6 * 228);
        eprintln!(
            "PPU source-store wholemax={maximum}T, own DImax={closed_maximum}T, +onePause66T; {injected} interrupt sites; minSP={min_sp:04X}"
        );
    }

    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT; optional TRACE_BASELINE_PROJECT is frozen f625"]
    fn mmc3_full_ciram_capture_masks_are_independent() {
        ...
        let current = resolve(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let mut variants = vec![(current.clone(), false, false), (current, true, false)];
        if let Ok(path) = std::env::var("TRACE_BASELINE_PROJECT") {
            variants.push((resolve(path), false, true));
        }
        let mut results = Vec::new();
        for (path, forced, old) in variants {
            let defs = load_wla_symbol_defs(&path.join("sms.sym"));
            let mut bus = SmsBus::new(std::fs::read(path.join("sms.sms")).unwrap(), 0xff);
            bus.write(0xfffc, 8);
            bus.write(0xcb28, 1); // actual host service, not boot's early return
            bus.write(0xc831, 1);
            bus.write(0xc833, 1);
            bus.write(0xdd3f, 1);
            bus.write(0xcb08, 0x28);
            bus.write(0xcb09, 0x18);
            bus.write(0xc824, 0);
            bus.write(0xd9f6, 2);
            bus.cart_ram[..0x800].fill(0x37);
            bus.ram[0x1d40..0x1d60].fill(0xff); // initial fixture source differs
            let invoke = |bus: &mut SmsBus, label: &str| {
                if forced && label != "rt_mmc3_capture_single" {
                    // Only the reference branch forces scans. Candidate intent
                    // comes exclusively from actual source writes below.
                    bus.ram[0x1d40..0x1d60].fill(0xff);
                }
                let mut cpu = Cpu::new();
                cpu.pc = defs[label].1;
                cpu.sp = 0xdff0;
                ...
                for _ in 0..300_000 {
                        ...
                        && cpu.sp == sp
                    {
                        assert_eq!(
                            ...
                            (af, bc, de, hl),
                            "HUD capture interrupted context"
                        );
                        interrupted = None;
                    }
                    let point = (cpu.pc, cpu.hl(), cpu.de());
                    if !old
                        && !forced
                        && label == "rt_mmc3_capture_hud"
                        && interrupted.is_none()
                        && cpu.iff1
                        && cpu.ei_pending == 0
                        && last_service != Some(point)
                    {
                        last_service = Some(point);
                        interrupted = Some([
                            cpu.pc,
                ...
                assert_eq!((cpu.pc, cpu.sp), (7, 0xdff2));
                assert!(!cpu.iff1);
                if !old && !forced && label == "rt_mmc3_capture_hud" {
                    assert!(host_services >= 16, "even clean HUD intent services host");
                    assert_eq!(bus.read(0xc8f9), 0, "BUSY excludes guest reentry");
                    eprintln!("independent HUD capture actual host-service points={host_services}");
                }
            };
            let mut states = Vec::new();
            for step in 0..12 {
                let source_store = |bus: &mut SmsBus, offset: u16, value| {
                    mmc3_test_ciram_store(bus, defs["rt_ppu_write"].1, 0x2000 + offset, value);
                };
                match step {
                    2 => source_store(&mut bus, 15, 0x38),
                    3 => source_store(&mut bus, 16, 0x39),
                    4 => {
                        source_store(&mut bus, 127, 0x91);
                        source_store(&mut bus, 127, 0x37);
                    }
                    6 => bus.write(0xc861, 0x21), // record-only, no CIRAM touch
                    8 => source_store(&mut bus, 2047, 0x40),
                    10 => {
                        for index in 0..2048u16 {
                            source_store(&mut bus, index, index as u8);
                        }
                    }
                    _ => {}
                }
                let label = match step {
                    1 | 3 | 5 | 9 => "rt_mmc3_capture_hud",
                    7 => "rt_mmc3_capture_single",
                    _ => "rt_mmc3_capture_playfield",
                };
                let other = if label == "rt_mmc3_capture_hud" {
                    0x1d40
                } else {
                    0x1d50
                };
                let before: [u8; 16] = bus.ram[other..other + 16].try_into().unwrap();
                invoke(&mut bus, label);
                if !old && !forced {
                    if label == "rt_mmc3_capture_single" {
                        assert!(bus.ram[0x1d50..0x1d60].iter().all(|&v| v == 0xff));
                    } else {
                        assert_eq!(&bus.ram[other..other + 16], &before);
                        let consumed = if other == 0x1d40 { 0x1d50 } else { 0x1d40 };
                        assert!(bus.ram[consumed..consumed + 16].iter().all(|&v| v == 0));
                    }
                }
                let mut state = bus.cart_ram[..0x1980].to_vec();
                state.extend_from_slice(&bus.ram[0x1c00..0x1d39]);
                state.extend([bus.read(0xdd3f), bus.read(0xc8f4), bus.read(0xc8ed)]);
                states.push(state);
            }
            results.push(states);
        }
        for result in &results[1..] {
            assert_eq!(
                &results[0], result,
                "independent snapshots/dirty bytes/records vs forced scan or old f625"
            );
        }
        eprintln!(
            "12 independent PF/HUD/single/record/restored/dense capture steps x {} controls",
            results.len()
        );
    }
    ...
    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT assembled MMC3_FULL_RUNTIME fixture"]
    fn mmc3_full_cell_capture_tracks_exact_physical_bytes() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        let invoke = |bus: &mut SmsBus, label: &str| {
            ...
            bus.write(cpu.sp, 7);
            bus.write(cpu.sp + 1, 0);
            let mut prefix_span = 0;
            let mut prefix_yields = 0;
            let mut prefix_max = 0;
            for _ in 0..500_000 {
                if cpu.pc == 7 {
                    break;
                }
                if label == "rt_mmc3_capture_playfield" && prefix_yields < 3 {
                    if cpu.iff1 && cpu.ei_pending == 0 {
                        prefix_max = prefix_max.max(prefix_span);
                        prefix_span = 0;
                        prefix_yields += 1;
                    }
                    if prefix_yields < 3 {
                        prefix_span += mmc3_row_nominal(&cpu, bus);
                    }
                }
                cpu.step(bus).unwrap();
                assert_eq!(bus.read(0xcb1d), 0, "{label}");
            ...
            assert_eq!((cpu.pc, cpu.sp), (7, 0xdff2), "{label}");
            assert!(!cpu.iff1, "capture returns DI");
            if label == "rt_mmc3_capture_playfield" {
                assert_eq!(prefix_yields, 3, "old-record32/19 and first8-byte group");
                assert!(
                    prefix_max <= 831,
                    "full capture prefix={prefix_max} nominal T"
                );
            }
            prefix_max
        };
        // Each physical byte changes in one of the eight sparse passes.
        // Dense/no-change passes also detect early-exit and stale-bit bugs.
        for pattern in 0..10 {
            for (ready, anchored) in [(0u8, 0u8), (0, 1), (1, 0), (1, 1)] {
                let mut bus = SmsBus::new(rom.clone(), 0xff);
                bus.write(0xfffc, 8);
                bus.write(0xc831, 1);
                bus.write(0xc833, ready);
                bus.write(0xd9f6, 2); // no old-display service in this direct test
                bus.write(0xcb08, 0xa8);
                bus.write(0xcb09, 0x18);
                bus.write(0xc824, 1);
                bus.write(0xc834, 0x23);
                bus.write(0xc835, 0x9f);
                bus.write(0xc836, 5);
                bus.write(0xc8f0, 0xc1); // last committed split, not pending C837
                bus.write(0xc837, 0xc0);
                bus.ram[..0x800].fill(0x65);
                bus.ram[0x1300..0x1600].fill(0x76);
                bus.ram[0x1c00..0x1d40].fill(0xa5);
                bus.write(0xdd3f, anchored);
                // This fixture initializes source/snapshot bytes directly.
                // Production interception is tested separately through PPU writes.
                bus.ram[0x1d40..0x1d60].fill(0xff);
                bus.ram[0x1d60..0x1d80].fill(0x87);
                bus.cart_ram[0x4000..0x6000].fill(0x98);
                let old_record: Vec<u8> = (0..51u8).map(|i| i.wrapping_mul(37)).collect();
                bus.cart_ram[0x1900..0x1933].copy_from_slice(&old_record);
                for index in 0..8u16 {
                    bus.write(0xc850 + index, index as u8);
                }
                let mut expected = [0u8; 256];
                for offset in 0..2048usize {
                    let old = (offset as u8).wrapping_mul(29);
                    let changed = pattern == 8 || (pattern < 8 && offset % 8 == pattern);
                    bus.cart_ram[0x800 + offset] = old;
                    // A live write restored before capture is clean; dirty
                    // history must not replace exact frozen-byte comparison.
                    bus.cart_ram[offset] = old ^ 0x40;
                    bus.cart_ram[offset] = old ^ u8::from(changed);
                    if changed {
                        expected[offset / 8] |= 1 << (offset % 8);
                    }
                }
                let live = bus.cart_ram[..0x800].to_vec();
                let prefix = invoke(&mut bus, "rt_mmc3_capture_playfield");
                if pattern == 8 && ready == 0 {
                    assert_eq!(prefix, 823, "dense ready0 capture-entry worst case");
                    eprintln!(
                        "READY0 PF capture entry:823 nominal T between closed service points"
                    );
                }
                assert_eq!(&bus.ram[0x1c00..0x1d00], &expected);
                if pattern == 8 && ready == 1 && anchored == 1 {
                    assert_eq!(prefix, 831, "READY1+ANCHOR1 capture-entry worst case");
                    eprintln!(
                        "READY1+ANCHOR1 PF capture entry:831 nominal T between closed service points"
                    );
                }
                assert_eq!(&bus.ram[0x1d00..0x1d33], &old_record);
                assert_eq!(
                    bus.read(0xdd33),
                    ready & anchored,
                    "validity requires committed source anchor"
                );
                assert_eq!(
                    bus.read(0xdd3f),
                    0,
                    "pending capture is not a committed source"
                );
                assert_eq!(bus.read(0xdd34), 0xc1, "old committed split identity");
                assert_eq!(&bus.cart_ram[..0x800], &live);
                assert_eq!(&bus.cart_ram[0x800..0x1000], &live);
                assert_eq!(bus.read(0xc8f4), 1, "the seeded record changed too");
                let evidence = bus.ram[0x1c00..0x1d40].to_vec();
                invoke(&mut bus, "rt_mmc3_capture_hud");
                assert_eq!(&bus.ram[0x1c00..0x1d40], &evidence);
                invoke(&mut bus, "rt_mmc3_capture_single");
                assert_eq!(&bus.ram[0x1c00..0x1d40], &evidence);
                assert!(bus.ram[..0x800].iter().all(|&b| b == 0x65));
                assert!(bus.ram[0x1300..0x1600].iter().all(|&b| b == 0x76));
                assert!(bus.ram[0x1d40..0x1d50].iter().all(|&b| b == 0));
                assert!(bus.ram[0x1d50..0x1d60].iter().all(|&b| b == 0xff));
                assert!(bus.ram[0x1d60..0x1d80].iter().all(|&b| b == 0x87));
                assert!(bus.cart_ram[0x4000..0x6000].iter().all(|&b| b == 0x98));
            }
        }
    }
    ...
    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT assembled MMC3_FULL_RUNTIME fixture"]
    fn mmc3_full_cell_reuse_matches_forced_resolution_lifetime() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        let defs = load_wla_symbol_defs(&path.join("sms.sym"));
        let rom = std::fs::read(path.join("sms.sms")).unwrap();
        let reuse = defs["_m3c_reuse_cell"].1;
        let mut outcomes = Vec::new();
        for forced in [false, true] {
            ...
            for page in 0..2usize {
                for offset in 0..0x3c0 {
                    bus.cart_ram[page * 1024 + offset] = ((offset + page * 3) & 7) as u8;
                }
                bus.cart_ram[page * 1024 + 0x3c0..page * 1024 + 0x400].fill(0xe4);
            }
            for offset in 0..256 {
                bus.write(0xc900 + offset, 0xe0);
            }
            ...
            bus.cart_ram[0x4000..0x6000].fill(0x96);
            bus.ram[0x1d40..0x1d60].fill(0xff); // direct fixture source initialization
            let mut hits = 0;
            let mut frames = Vec::new();
            for frame in 0..12 {
                // Captured producer changes, not direct edits to committed
                // record metadata: exact -> stable -> ordinary changed cells
                // -> mapping/geometry changes -> off/re-enable -> split.
                match frame {
                    2 => bus.write(0xc861, 0x21),
                    3 => {
                        let value = bus.cart_ram[5] ^ 1;
                        mmc3_test_ciram_store(&mut bus, defs["rt_ppu_write"].1, 0x2005, value);
                    }
                    4 => bus.write(0xc835, 1),
                    5 => bus.write(0xc824, 0),
                    6 => bus.write(0xc850, 4),
                    7 => bus.write(0xcb09, 0),
                    8 => bus.write(0xcb09, 0x18),
                    9 => {
                        let value = bus.cart_ram[0x3c0] ^ 3;
                        mmc3_test_ciram_store(&mut bus, defs["rt_ppu_write"].1, 0x23c0, value);
                    }
                    10 => bus.write(0xc835, 0x21),
                    _ => {}
                }
                for label in [
                    ...
                    bus.write(0xc831, 2);
                    if frame == 11 && label == "rt_mmc3_frame_present" {
                        bus.write(0xc837, 0xc1);
                        bus.write(0x994e, 5); // independent HUD fine-X
                    }
                    let mut cpu = Cpu::new();
                    cpu.pc = defs[label].1;
                    cpu.sp = 0xdff0;
                    bus.write(cpu.sp, 7);
                    bus.write(cpu.sp + 1, 0);
                    let mut result_pc = None;
                    let mut publishing = false;
                    let mut host_fired = false;
                    let mut min_sp = cpu.sp;
                    for _ in 0..4_000_000 {
                        if cpu.pc == 7 {
                            break;
                        }
                        if result_pc == Some(cpu.pc) {
                            hits += usize::from(cpu.f & 1 == 0);
                            result_pc = None;
                        }
                        if cpu.pc == reuse {
                            let ret = u16::from_le_bytes([bus.read(cpu.sp), bus.read(cpu.sp + 1)]);
                            if forced {
                                // Test-only SCF/RET affordance. The helper's
                                // strict-miss ABI leaves all registers scratch
                                // and performs no NEEDED/publication writes.
                                cpu.f |= 1;
                                cpu.pc = ret;
                                cpu.sp += 2;
                                continue;
                        ...
                        if cpu.pc == defs["rt_mmc3_commit_display"].1 {
                            publishing = true;
                            let ret = u16::from_le_bytes([bus.read(cpu.sp), bus.read(cpu.sp + 1)]);
                            assert_eq!(
                                bus.read(ret),
                                0xc3,
                                "publisher return precedes anchor completion"
                            );
                        }
                        if frame == 3
                            && publishing
                            ...
                            && cpu.ei_pending == 0
                        {
                            assert_eq!(bus.read(0xdd3f), 0, "not anchored during publisher");
                            bus.write(0xcb28, 1);
                            let saved = (cpu.pc, cpu.af(), cpu.bc(), cpu.de(), cpu.hl(), cpu.sp);
                            cpu.sp -= 2;
                            bus.write(cpu.sp, cpu.pc as u8);
                            bus.write(cpu.sp + 1, (cpu.pc >> 8) as u8);
                            cpu.pc = 0x38;
                            ...
                            assert_eq!(
                                (cpu.pc, cpu.af(), cpu.bc(), cpu.de(), cpu.hl(), cpu.sp),
                                saved
                            );
                            assert_eq!(bus.read(0xdd3f), 0);
                            assert_eq!(bus.read(0xc8f9), 0, "host cannot enter guest");
                            host_fired = true;
                        }
                        cpu.step(&mut bus).unwrap();
                        min_sp = min_sp.min(cpu.sp);
                        assert_eq!(bus.read(0xcb1d), 0, "frame{frame} {label}");
                        assert!(cpu.sp >= NATIVE_STACK_FLOOR);
                    }
                    assert_eq!((cpu.pc, cpu.sp), (7, 0xdff2), "frame{frame} {label}");
                    if frame == 3 && label == "rt_mmc3_frame_present" {
                        assert!(host_fired, "host IRQ while extra publisher return is live");
                        eprintln!("publisher return + host IRQ minimumSP={min_sp:04X}");
                    }
                }
                if frame == 7 {
                    assert_eq!(bus.read(0xc833), 0);
                    assert_eq!(bus.read(0xdd33), 0, "source-off invalidates reuse");
                    assert_eq!(bus.read(0xdd3f), 0);
                } else {
                    ...
                    assert_eq!(
                        bus.read(0xdd3f),
                        1,
                        "full/stable/exact completion anchors source"
                    );
                }
                // Slot numbers can differ without changing pixels. Resolve
                // all28 rendered rows to actual32-byte patterns plus attribute
                // bits, independently of cache/hash metadata. The remaining
                // rows are padding cell0, not additional source pixels.
                let mut patterns = Vec::new();
                for cell in bus.vram[0x3700..0x3e00].as_chunks::<2>().0 {
                    let entry = u16::from_le_bytes([cell[0], cell[1]]);
                    let slot = usize::from(entry & 511);
                    patterns.extend_from_slice(&bus.vram[slot * 32..slot * 32 + 32]);
                    patterns.push(cell[1] & !1);
                }
                assert!(bus.vram[0x3e00..0x3f00].iter().all(|&b| b == 0));
                frames.push((patterns, bus.cram, bus.vdp_regs));
                assert!(bus.cart_ram[0x4000..0x6000].iter().all(|&b| b == 0x96));
            }
            if !forced {
                assert!(hits > 0, "the sequence must exercise successful reuse");
                eprintln!("cell lifetime sequence successful reuse hits={hits}");
            }
            outcomes.push(frames);
        }
        assert_eq!(
            outcomes[0], outcomes[1],
            "captured reuse vs forced resolution"
        );
    }

    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT assembled MMC3_FULL_RUNTIME fixture"]
    fn mmc3_full_cell_reuse_rejects_cancelled_unpublished_capture() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        let chr = asm
            ...
            * BANK_SIZE;
        let mut rom = std::fs::read(path.join("sms.sms")).unwrap();
        rom[chr..chr + 48].fill(0);
        rom[chr + 16..chr + 24].fill(0xff); // tile1=color1 on every pixel
        rom[chr + 40..chr + 48].fill(0xff); // tile2=color2, independently dirty
        let mut outcomes = Vec::new();
        for (forced, change_y) in [(false, false), (false, true), (true, false), (true, true)] {
            let mut bus = SmsBus::new(rom.clone(), 0xff);
            bus.write(0xfffc, 8);
            bus.write(0xc831, 2);
            bus.write(0xcb28, 1);
            bus.write(0xcb08, 0x28); // NMI disabled: no game handler is substituted
            bus.write(0xcb09, 0x18);
            bus.write(0xc824, 1);
            bus.write(0xcb03, 0x24);
            for index in 0..8 {
                bus.write(0xc850 + index, index as u8);
            }
            for index in 0..256 {
            ...
            let invoke = |bus: &mut SmsBus, label: &str, ppu: Option<(u8, u8)>| {
                let mut cpu = Cpu::new();
                cpu.pc = defs[label].1;
                cpu.sp = 0xdff0;
                if let Some((register, value)) = ppu {
                    cpu.b = register;
                    cpu.a = value;
                }
                bus.write(cpu.sp, 7);
                ...
                for _ in 0..3_000_000 {
                    ...
                    if forced && cpu.pc == defs["_m3c_reuse_cell"].1 {
                        cpu.pc = u16::from_le_bytes([bus.read(cpu.sp), bus.read(cpu.sp + 1)]);
                        cpu.sp += 2;
                        cpu.f |= 1;
                        continue;
                    }
            ...
            // Initial fixture RAM was prepared without guest stores.
            bus.ram[0x1d40..0x1d60].fill(0xff);
            // A is coherently published, with an ordinary PF row at y=5.
            invoke(&mut bus, "rt_mmc3_capture_playfield", None);
            invoke(&mut bus, "rt_mmc3_capture_single", None);
            bus.write(0xc837, 0xc0);
            invoke(&mut bus, "rt_mmc3_frame_present", None);
            let committed_a = bus.vram;
            const X: usize = 5 * 32 + 4;
            const Y: usize = 5 * 32 + 8;
            bus.write(0xcb0f, 0x20 | ((X >> 8) as u8));
            bus.write(0xcb10, X as u8);
            invoke(&mut bus, "rt_ppu_write", Some((7, 1)));
            invoke(&mut bus, "rt_mmc3_capture_playfield", None); // pending B
            assert_eq!(bus.vram, committed_a, "captureB was not publication");
            bus.write(0xc832, 1); // pending split, not yet asserted IRQ
            bus.write(0xc822, 1);
            bus.write(0xc823, 0);
            bus.write(0xc831, 0); // main owns producer again
            invoke(&mut bus, "rt_ppu_write", Some((1, 0)));
            invoke(&mut bus, "rt_ppu_write", Some((1, 0x18)));
            assert_eq!(bus.read(0xc8f6), 1, "real MASK off/on cancellation intent");
            bus.write(0xc825, 1); // actual cooperative-wait scheduler admission
            bus.frame_int_pending = true;
            invoke(&mut bus, "rt_mmc3_sms_interrupt", None);
            assert_eq!((bus.read(0xc832), bus.read(0xc8f6)), (0, 0));
            assert_eq!(bus.vram, committed_a, "cancelledB was never committed");
            if change_y {
                bus.write(0xcb0f, 0x20 | ((Y >> 8) as u8));
                bus.write(0xcb10, Y as u8);
                invoke(&mut bus, "rt_ppu_write", Some((7, 2)));
            }
            bus.write(0xc831, 2);
            invoke(&mut bus, "rt_mmc3_capture_playfield", None); // source C
            assert_eq!(
                bus.ram[0x1c00 + X / 8] & (1 << (X & 7)),
                0,
                "X is clean relative to abandonedB, NOT committedA"
            );
            invoke(&mut bus, "rt_mmc3_capture_single", None);
            bus.write(0xc837, 0xc0);
            invoke(&mut bus, "rt_mmc3_frame_present", None);
            assert_mmc3_needed_covers_committed_nt(&bus);
            let slot = usize::from(bus.vram[0x3700 + X * 2]);
            let pattern: [u8; 32] = bus.vram[slot * 32..slot * 32 + 32].try_into().unwrap();
            assert_eq!(bus.read(0xdd3f), 1, "corrected publication anchors sourceC");
            bus.write(0xc831, 1);
            invoke(&mut bus, "rt_mmc3_capture_playfield", None);
            invoke(&mut bus, "rt_mmc3_capture_single", None);
            bus.write(0xc837, 0xc0);
            assert_eq!(bus.read(0xdd33), 1, "next capture can use committedC");
            assert_eq!(bus.read(0xc8f4), 0);
            invoke(&mut bus, "rt_mmc3_frame_present", None);
            assert_eq!(
                bus.read(0xc831),
                1,
                "exact reuse resumes after corrected publication"
            );
            assert_eq!(bus.read(0xdd3f), 1);
            outcomes.push(pattern);
        }
        let expected = std::array::from_fn(|index| if index % 4 == 0 { 0xff } else { 0 });
        assert_eq!(
            outcomes[2], expected,
            "independent currentNES color1 pattern"
        );
        assert_eq!(
            outcomes[3], expected,
            "forced full with independently changedY"
        );
        assert_eq!(
            outcomes[0], expected,
            "cancelled pending source must not permit exact-packet reuse"
        );
        assert_eq!(
            outcomes[1], expected,
            "cancelled pending source must not anchor committed-cell reuse"
        );
    }

    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT assembled MMC3_FULL_RUNTIME fixture"]
    fn mmc3_full_cell_eligibility_matches_physical_dependencies() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        let seed = |bus: &mut SmsBus| {
            bus.write(0xfffc, 8);
            bus.write(0xc831, 2);
            bus.write(0xc833, 1);
            bus.ram[0x1c00..0x1d40].fill(0);
            bus.cart_ram[0x1900..0x1980].fill(0);
            for base in [0x9900, 0xdd00] {
                for index in 0..8 {
                    bus.write(base + index, index as u8);
                }
                bus.write(base + 8, 0xa8);
                bus.write(base + 9, 0x18);
                bus.write(base + 15, 1);
            }
            bus.write(0xdd33, 1);
            bus.write(0xdd34, 0xc0);
            bus.write(0xc837, 0xc0);
            bus.write(0xc842, 5);
            bus.write(0xc848, 0);
            bus.write(0xc849, 0);
        ...
        let mut bus = SmsBus::new(rom, 0xff);
        // Each meaningful frame dependency changes alone. Nonselected CHR,
        // fine-X/raw-scroll/palette are deliberately NOT pattern dependencies.
        for (address, xor, expected) in [
            (0xdd33, 1, false),
            (0xc833, 1, false),
            (0xdd34, 1, false),
            (0x9909, 0x18, false),
            ...
            seed(&mut bus);
            let old = bus.read(address);
            bus.write(address, old ^ xor);
            invoke(&mut bus, "_m3c_frame_eligibility");
            assert_eq!(bus.read(0xdd35) != 0, expected, "dependency{address:04X}");
        }
        for (record, row, fine, offset, split, expected) in [
            (0, 0, 0, 0, 0xc0, false),
            (0x40, 5, 0, 0, 0xc0, false),
            (0, 5, 0, 1, 0xc0, false),
            (0, 23, 0, 0, 0xc0, true),
            (0, 24, 0, 0, 0xc1, false),
            (0, 24, 7, 0, 0xc1, true),
            ...
            seed(&mut bus);
            bus.write(0xdd35, 1);
            bus.write(0xc849, record);
            bus.write(0xc842, row);
            bus.write(0xc848, fine);
            bus.write(0xc8ea, offset);
            bus.write(0xc837, split);
            invoke(&mut bus, "_m3c_row_eligibility");
            assert_eq!(bus.read(0xdd36) != 0, expected, "row{row}/fine{fine}");
        }
        let physical = |v: u16, mirror: u8| -> usize {
            let page = if mirror == 0 {
                (v >> 10) & 1
            } else {
                (v >> 11) & 1
            };
            usize::from(page * 1024 + (v & 1023))
        };
        let mut cells = 0;
        for mirror in [0u8, 1] {
            for old_x in [0u16, 1, 31] {
                for new_x in [0u16, 1, 31] {
                    for old_page in [0u16, 1] {
                        for new_page in [0u16, 1] {
                            seed(&mut bus);
                            bus.write(0xdd0c, old_x as u8);
                            bus.write(0xdd0d, (old_page << 2) as u8);
                            bus.write(0x990c, new_x as u8);
                            bus.write(0x990d, (new_page << 2) as u8);
                            bus.write(0xdd0f, mirror);
                            bus.write(0x990f, mirror);
                            invoke(&mut bus, "_m3c_frame_eligibility");
                            invoke(&mut bus, "_m3c_row_eligibility");
                            assert_eq!(bus.read(0xdd36), 1);
                            for col in 0..32u16 {
                                let old_v =
                                    0xa0 | col | ((old_page ^ u16::from(col < old_x)) << 10);
                                let new_v =
                                    0xa0 | col | ((new_page ^ u16::from(col < new_x)) << 10);
                                let tile = physical(new_v, mirror);
                                let attr_v = (new_v & 0x0c00)
                                    | 0x3c0
                                    | ((new_v >> 4) & 0x38)
                                    | ((new_v >> 2) & 7);
                                let attr = physical(attr_v, mirror);
                                let nt = 5 * 64 + col * 2;
                                bus.write(0xc846, new_v as u8);
                                bus.write(0xc847, (new_v >> 8) as u8);
                                bus.write(0xc858, nt as u8);
                                bus.write(0xc859, 0xa0 + (nt >> 8) as u8);
                                bus.write(0xcc00 + nt, col as u8 ^ 0xb7);
                                invoke(&mut bus, "_m3r_row_addresses");
                                for dirty in [None, Some(tile), Some(attr)] {
                                    bus.ram[0x1c00..0x1d00].fill(0);
                                    if let Some(index) = dirty {
                                        bus.ram[0x1c00 + index / 8] |= 1 << (index & 7);
                                    }
                                    let evidence = bus.ram[0x1c00..0x1d40].to_vec();
                                    let needed = bus.ram[0x17e0..0x1800].to_vec();
                                    let cpu = invoke(&mut bus, "_m3c_reuse_cell");
                                    let hit = physical(old_v, mirror) == tile && dirty.is_none();
                                    assert_eq!(
                                        cpu.f & 1 == 0,
                                        hit,
                                        "old{old_v:04X} new{new_v:04X} mirror{mirror} dirty{dirty:?}"
                                    );
                                    if hit {
                                        assert_eq!(cpu.a, col as u8 ^ 0xb7);
                                    }
                                    assert_eq!(&bus.ram[0x1c00..0x1d40], &evidence);
                                    assert_eq!(&bus.ram[0x17e0..0x1800], &needed);
                                    assert_eq!(
                                        u16::from_le_bytes([bus.read(0xc846), bus.read(0xc847)]),
                                        new_v
                                    );
                                    assert_eq!(bus.read(0xc849), 0);
                                    cells += 1;
                                }
                            }
                        }
        ...
        // The final fine-Y row's committed bytes skip the software stack:
        // A6xx mapsD2xx; A7xx mapsD6xx, neverD3xx. Exercise both ends directly.
        for row in [27u16, 28] {
            for col in [0u16, 31] {
                seed(&mut bus);
                bus.ram[0x1300..0x1600].fill(0x76);
                bus.write(0xc842, row as u8);
                bus.write(0xc848, 7);
                bus.write(0xc837, 0);
                bus.write(0xdd34, 0);
                bus.write(0xdd0d, 0x70);
                bus.write(0x990d, 0x70);
                invoke(&mut bus, "_m3c_frame_eligibility");
                invoke(&mut bus, "_m3c_row_eligibility");
                assert_eq!(bus.read(0xdd36), 1);
                let v = row * 32 + col;
                let nt = row * 64 + col * 2;
                let native = if nt < 0x700 {
                    0xcc00 + nt
                } else {
                    0xd600 + nt - 0x700
                };
                bus.write(0xc846, v as u8);
                bus.write(0xc847, (v >> 8) as u8);
                bus.write(0xc858, nt as u8);
                bus.write(0xc859, 0xa0 + (nt >> 8) as u8);
                bus.write(native, 0xb7);
                invoke(&mut bus, "_m3r_row_addresses");
                let cpu = invoke(&mut bus, "_m3c_reuse_cell");
                assert_eq!((cpu.f & 1, cpu.a), (0, 0xb7), "committed{native:04X}");
                assert!(bus.ram[0x1300..0x1600].iter().all(|&b| b == 0x76));
            }
        }
        eprintln!("cell dependency vectors={cells}, frame dependencies=24, row boundaries=7");
    }

    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT assembled MMC3_FULL_RUNTIME fixture"]
    fn mmc3_full_cell_capture_bounds_host_and_pause_service() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        let defs = load_wla_symbol_defs(&path.join("sms.sym"));
        let rom = std::fs::read(path.join("sms.sms")).unwrap();
        let entry = defs["_m3c_capture_ciram"].1;
        let group = defs["_m3c_copy_group"].1;
        let mut max_span = 0u64;
        // Baseline covers every possible dirty mask in one2048-byte capture.
        // Then Pause interrupts each executed opcode boundary in that kernel.
        let mut injections = vec![(0, None), (1, None), (2, None)];
        let mut run = 0;
        while run < injections.len() {
            let (pattern, pause) = injections[run];
            let baseline = pause.is_none();
            let mut points = std::collections::BTreeSet::new();
            let mut bus = SmsBus::new(rom.clone(), 0xff);
            bus.write(0xfffc, 8);
            bus.write(0xfffe, 6);
            ...
            bus.write(0xd9f6, 2);
            bus.write(0xc8ed, 1);
            bus.ram[0x1c00..0x1d80].fill(0x87);
            bus.ram[0x1d40..0x1d50].fill(match pattern {
                0 => 0xff,
                1 => 0,
                _ => 0x55,
            });
            ...
            for index in 0..2048usize {
                let mask = if pattern == 1 || (pattern == 2 && (index / 16) & 1 != 0) {
                    0
                } else {
                    (index / 8) as u8 ^ 0xff
                };
                let value = (index as u8).wrapping_mul(29);
                bus.cart_ram[index] = value;
                bus.cart_ram[0x800 + index] = value ^ ((mask >> (index & 7)) & 1);
            }
            let guest = bus.ram[..0x800].to_vec();
            let mut cpu = Cpu::new();
            cpu.pc = entry;
            ...
            let mut last_service = None;
            // Dense capture now exposes400 rather than256 real host-service
            // points. Bound the added handler executions, not the DI deadline.
            for _ in 0..350_000 {
                if cpu.pc == 7 {
                    break;
                }
                if let Some([pc, af, bc, de, hl, sp]) = interrupted
                    && cpu.pc == pc
                    && cpu.sp == sp
                {
                    assert_eq!(
                ...
                if interrupted.is_none() {
                    if baseline {
                        points.insert(cpu.pc);
                    }
                    let service = (cpu.pc, cpu.hl(), cpu.de());
                    let host = baseline
                        && cpu.iff1
                        && cpu.ei_pending == 0
                        && last_service != Some(service);
                    let nmi = !fired && pause == Some(cpu.pc);
                    if host || nmi {
                        if host {
                            let copied = cpu.de() - 0x8800;
                            assert!(copied >= last_copied && copied - last_copied <= 128);
                            last_copied = copied;
                            last_service = Some(service);
                ...
                if interrupted.is_none() && baseline {
                    span += mmc3_row_nominal(&cpu, &mut bus);
                }
                cpu.step(&mut bus).unwrap();
                assert_eq!(bus.read(0xcb1d), 0);
                assert!(cpu.sp >= NATIVE_STACK_FLOOR);
            ...
            assert_eq!(
                (cpu.pc, cpu.sp, cpu.hl(), cpu.de()),
                (7, 0xdff2, 0x8800, 0x9000)
            );
            assert!(!cpu.iff1);
            for byte in 0..256usize {
                let expected = if pattern == 1 || (pattern == 2 && (byte / 2) & 1 != 0) {
                    0
                } else {
                    byte as u8 ^ 0xff
                };
                assert_eq!(bus.ram[0x1c00 + byte], expected);
            }
            assert_eq!(&bus.cart_ram[..0x800], &bus.cart_ram[0x800..0x1000]);
            assert_eq!(&bus.ram[..0x800], &guest);
            assert!(bus.ram[0x1300..0x1600].iter().all(|&b| b == 0x76));
            assert!(bus.ram[0x1d00..0x1d39].iter().all(|&b| b == 0x87));
            assert!(bus.ram[0x1d3b..0x1d40].iter().all(|&b| b == 0x87));
            assert!(bus.ram[0x1d40..0x1d50].iter().all(|&b| b == 0));
            assert!(bus.ram[0x1d50..0x1d80].iter().all(|&b| b == 0x87));
            assert!(bus.cart_ram[0x4000..0x6000].iter().all(|&b| b == 0x96));
            assert_eq!(
                (bus.slot_bank[1], bus.slot_bank[2], bus.read(0xfffc)),
                (6, 9, 8)
            ...
            if baseline {
                ...
                assert_eq!(points.contains(&group), pattern != 1);
                injections.extend(points.iter().copied().map(|pc| (pattern, Some(pc))));
            } else {
                assert!(fired);
                assert_eq!(bus.read(0xcb2e), 4);
            }
        ...
        // Bound only the new capture's closed service interval, excluding
        // handler execution; the actual stock-core observer owns raster proof.
        assert!(
            max_span <= 831,
            "new capture service interval={max_span} nominal T"
        );
        eprintln!(
            "capture dense/clean/mixed;400+16+272 host services/{}Pause sites; max closed span={max_span} nominal T",
            injections.len() - 3
        );
    }
    ...
    fn mmc3_full_renderer_accepts_256_keys_and_traps_the_257th() {
        ...
        for overflow in [false, true] {
            let mut bus = SmsBus::new(rom.clone(), 0xff);
            bus.ram[0x1d40..0x1d60].fill(0xff); // direct initial source fixture
            bus.write(0xfffc, 8);
            bus.write(0xc831, 2);
            bus.write(0xcb08, 0xa8);
            bus.write(0xcb09, 0x18);
            ...
            for page in 0..2u16 {
                for offset in 0..0x3c0 {
                    bus.write(0x8000 + page * 0x400 + offset, offset as u8);
                }
            }
            for offset in 0..0x100 {
                bus.write(0xc900 + offset, 0xe0);
    ...
    fn mmc3_full_fast_residency_restart_retains_staged_payloads() {
        ...
        let invoke = |bus: &mut SmsBus, label: &str, force_reserve: bool| {
            ...
            for _ in 0..4_000_000 {
                ...
                if reuse_return == Some(cpu.pc) {
                    reuse_hits += usize::from(cpu.f & 1 == 0);
                    reuse_return = None;
                }
                if defs
                    .get("_m3c_reuse_cell")
                    .is_some_and(|&(_, pc)| cpu.pc == pc)
                {
                    let ret = u16::from_le_bytes([bus.read(cpu.sp), bus.read(cpu.sp + 1)]);
                    if force_reserve {
                        cpu.pc = ret;
                        cpu.sp += 2;
                        cpu.f |= 1; // independent reserve/full-resolution reference
                        continue;
                    }
                    reuse_return = Some(ret);
                }
                cpu.step(bus).unwrap();
                assert_eq!(bus.read(0xcb1d), 0, "{label}");
                assert!(cpu.sp >= NATIVE_STACK_FLOOR);
            ...
            assert_eq!(cpu.pc, 7);
            if label == "rt_mmc3_frame_render" && !force_reserve && bus.read(0xdd35) != 0 {
                assert!(reuse_hits > 0, "restart sequence must actually reuse cells");
                eprintln!("restarts={restarts}, successful cell reuse hits={reuse_hits}");
            }
            restarts
        };
        let mut bus = SmsBus::new(rom.clone(), 0xff);
        ...
        if defs.contains_key("_m3c_reuse_cell") {
            // Keep the selected CHR map identical: sparse attribute changes
            // force allocation/restart while clean later PF rows still reuse
            // committed slots. The reference resolves every cell in reserve.
            for attribute in [0x55, 0xaa] {
                mmc3_test_ciram_store(&mut bus, defs["rt_ppu_write"].1, 0x23c0, attribute);
                invoke(&mut bus, "rt_mmc3_capture_playfield", false);
                invoke(&mut bus, "rt_mmc3_capture_single", false);
                let mut reference = SmsBus::new(rom.clone(), 0xff);
                reference.ram = bus.ram;
                reference.cart_ram = bus.cart_ram;
                reference.vram = bus.vram;
                reference.vdp_regs = bus.vdp_regs;
                reference.cram = bus.cram;
                reference.write(0xfffc, 8);
                invoke(&mut reference, "rt_mmc3_frame_render", true);
                assert_eq!(invoke(&mut bus, "rt_mmc3_frame_render", false), 1);
                assert_eq!(bus.read(0xdd35), 1, "selected source map remains reusable");
                assert_mmc3_needed_covers_committed_nt(&bus);
                assert_eq!(bus.vram, reference.vram, "attribute={attribute:02X}");
                assert_eq!(bus.cram, reference.cram);
                assert_eq!(bus.vdp_regs, reference.vdp_regs);
            }
        }
    ...
    fn mmc3_full_common_ring_matches_source_pixels() {
        ...
        for coarse in [0u8, 1, 31] {
            for fine_x in 0..8u8 {
                for fine_y in [0u8, 7] {
                    cases.push((coarse, fine_x, fine_y, false));
                }
            }
        }
        // Preserve the original192 frame chain cases, and independently vary
        // both captured records across every fine-X/Y combination. Live mapper
        // state and the primary record must never substitute for the HUD copy.
        for fine_x in 0..8u8 {
            for fine_y in 0..8u8 {
                cases.push(([0, 1, 31][usize::from(fine_y % 3)], fine_x, fine_y, true));
            }
        }
        for (coarse, fine_x, fine_y, independent_hud) in cases {
            ...
            let hud_ctrl = ctrl ^ if independent_hud { 0x10 } else { 0 };
            let hud_mirroring = mirroring ^ u8::from(independent_hud);
            let mut bus = SmsBus::new(rom.clone(), 0xff);
            bus.ram[0x1d40..0x1d60].fill(0xff); // direct initial source fixture
            bus.write(0xfffc, 8);
            bus.write(0xc831, 2);
            ...
            invoke(&mut bus, "rt_mmc3_capture_playfield");
            bus.write(0xcb08, hud_ctrl);
            bus.write(0xc824, hud_mirroring);
            if independent_hud {
                for index in 0..8u16 {
                    let mapped = if fine_x & 1 == 0 { index } else { index ^ 4 };
                    bus.write(0xc850 + index, ((mapped + 2) & 7) as u8);
                }
            }
            ...
            for (next_coarse, stable, reenable) in [
                ...
                for y in 0..224usize {
                    ...
                    let source_mirroring = if hud { hud_mirroring } else { mirroring };
                    let source_ctrl = if hud { hud_ctrl } else { ctrl };
                    ...
                    for x in 0..256usize {
                        ...
                        let offset = if source_mirroring == 0 {
                            nt & 0x7ff
                        } else {
                            (nt & 0x3ff) | ((nt & 0x800) >> 1)
                        };
                        ...
                        let pattern = (usize::from(source_ctrl & 0x10) << 8) + tile * 16;
                        ...
                        let attr_nt = (nt & 0xc00) | 0x3c0 | ((nt >> 4) & 0x38) | ((nt >> 2) & 7);
                        let attr_offset = if source_mirroring == 0 {
                            attr_nt & 0x7ff
                        } else {
                        ...
                        let actual = (0..4).fold(0u8, |value, plane| {
                            value | (((bus.vram[addr + plane] >> (7 - sms_x % 8)) & 1) << plane)
                        });
                        let masked = x < 8 && mask & 2 == 0;
                        if !masked {
                            assert_eq!(
                                actual, expected,
                                "coarse{coarse} fx{fine_x} fy{fine_y} split{split} reload{reload} independentHUD{independent_hud} at{x},{y}"
                            );
                        }
    ...
    // Exact nominal costs for the row/source subset (Zilog UM0080 and pinned
    // GPGX cc_op/cc_cb), not Cpu.cycles. Unknown opcodes fail closed.
    fn mmc3_row_nominal(cpu: &Cpu, bus: &mut SmsBus) -> u64 {
        let op = bus.read(cpu.pc);
        match op {
            0x00 | 0x07 | 0x0f | 0x17 | 0x1f | 0xeb | 0xf3 | 0xfb => 4,
            0x01 | 0x11 | 0x21 | 0x36 => 10,
            0x10 => {
                if cpu.b == 1 {
                    8
                } else {
            ...
            0x06 | 0x0e | 0x16 | 0x1e | 0x26 | 0x2e | 0x3e | 0xe6 | 0xee | 0xf6 | 0xfe | 0xc6
            | 0xd6 | 0xce => 7,
            0x13 | 0x23 => 6,
            0x0a | 0x12 | 0x1a => 7,
            0x04 | 0x0c | 0x14 | 0x1c | 0x24 | 0x2c | 0x3c | 0x3d => 4,
            0x09 | 0x19 | 0x29 => 11,
            0x22 | 0x2a => 16,
            0x32 | 0x3a => 13,
            0x40..=0x75 | 0x77..=0x7f | 0x80..=0xbf => {
                if op & 7 == 6 || (op < 0x80 && (op >> 3) & 7 == 6) {
                    7
                } else {
                    4
                }
            ...
            0xc3 | 0xc2 | 0xca | 0xd2 | 0xda | 0xe2 => 10,
            0xc5 | 0xd5 | 0xe5 | 0xf5 => 11,
            0xc1 | 0xd1 | 0xe1 | 0xf1 | 0xc9 => 10,
            0xcd => 17,
            0xc4 => {
                if cpu.f & 0x40 == 0 {
                    17
            ...
            0xcb => {
                let sub = bus.read(cpu.pc.wrapping_add(1));
                assert!(
                    sub >= 0x40 || sub == 0x38 || sub == 0x25,
                    "unaccounted CB{sub:02X}"
                );
                if sub & 7 == 6 {
                    if sub < 0x80 { 12 } else { 15 }
                } else {
                    8
                }
            }
            0xed => {
                ...
                match sub {
                    ...
                    _ => panic!("unaccounted ED{sub:02X}"),
                }
            }
            _ => panic!("unaccounted row opcode{op:02X} at{:04X}", cpu.pc),
        }
    }

    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT assembled MMC3 row-address kernel"]
    fn mmc3_full_row_address_kernel_matches_frozen_sources() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        for label in [
            ...
            assert_eq!(defs[label].0, 0, "kernel must remain fixed-slot code");
        }
        let mut bus = SmsBus::new(rom, 0xff);
        bus.write(0xfffc, 8);
        bus.write(0xfffe, 17);
        bus.write(0xcb14, 17);
        bus.write(0xcb28, 1);
        bus.write(0xc831, 2); // Frozen producer: host service cannot enter guest code.
        bus.write(0xc833, 1);
        bus.write(0xc8f2, 0x26);
        bus.write(0xcb03, 0xa1);
        for offset in 0..0x1000u16 {
            bus.write(
                0x8800 + offset,
                (offset as u8).wrapping_mul(53) ^ (offset >> 6) as u8,
            );
        }
        for record in [0u16, 0x40] {
            for index in 0..8u16 {
                bus.write(
                    0x9900 + record + index,
                    (index as u8 * 29).wrapping_add(record as u8 + 3),
                );
            }
        }
        let guest = bus.ram[..0x800].to_vec();
        let protected = bus.ram[0x1300..0x1600].to_vec();
        let mapping = (bus.slot_bank, bus.read(0xfffc));
        let segment = |bus: &mut SmsBus, start: &str, stop: u16| {
            let mut cpu = Cpu::new();
            cpu.pc = defs[start].1;
            cpu.sp = 0xdff0;
            cpu.a = 0xa7;
            ...
            for _ in 0..500 {
                ...
                ticks += mmc3_row_nominal(&cpu, bus);
                cpu.step(bus).unwrap();
                assert_eq!(bus.read(0xcb1d), 0, "{start} trapped");
            }
            assert_eq!(cpu.pc, stop, "{start} did not finish");
            assert_eq!(cpu.sp, if stop == 7 { 0xdff2 } else { 0xdff0 });
            assert!(!cpu.iff1, "primary kernel must not open an IRQ boundary");
            (cpu, ticks)
        };
        let cache = |bus: &SmsBus| {
            [
                bus.ram[0x85b],
                bus.ram[0x85c],
                bus.ram[0x8ee],
        ...
        let mirrored = |v: u16, horizontal: bool| {
            if horizontal {
                (v & 0x3ff) | ((v & 0x800) >> 1)
            } else {
                v & 0x7ff
        ...
        for record in [0u16, 0x40] {
            for horizontal in [false, true] {
                // Opposite record properties expose accidental live/PF reuse.
                bus.write(0x990f + record, u8::from(horizontal));
                bus.write(0x990f + (record ^ 0x40), u8::from(!horizontal));
                bus.write(0xc824, u8::from(!horizontal));
                for nt in 0..4u16 {
                    for row in 0..32u16 {
                        for start_x in [0u16, 1, 7, 15, 31] {
                            let ctrl = if start_x & 1 == 0 { 0x08 } else { 0x18 };
                            bus.write(0x9908 + record, ctrl);
                            bus.write(0x9908 + (record ^ 0x40), ctrl ^ 0x10);
                            let frozen = bus.cart_ram[0x800..0x1980].to_vec();
                            let mut v = nt * 1024 + row * 32 + start_x;
                            for col in 0..32 {
                                bus.write(0xc846, v as u8);
                                bus.write(0xc847, (v >> 8) as u8);
                                bus.write(0xc849, record as u8);
                                bus.write(0xc8e9, 0);
                                if col == 0 || v & 31 == 0 {
                                    let (mut cpu, ticks) =
                                        segment(&mut bus, "_m3r_row_addresses", 7);
                                    max_setup = max_setup.max(ticks);
                                    let base = if record == 0 { 0x8800 } else { 0x9000 };
                                    let attr = (v & 0xc00) | 0x3c0 | ((v >> 4) & 0x38);
                                    assert_eq!(
                                        cache(&bus),
                                        [
                                            ((base + mirrored(v, horizontal)) >> 8) as u8,
                                            ((base + mirrored(attr, horizontal)) >> 8) as u8,
                                            attr as u8,
                                            v as u8 & 0x40,
                                        ],
                                        "record{record:02X} v{v:04X}"
                                    );
                                    // Execute the real host vector at the same closed boundary
                                    // as the row/wrap caller, while cached addresses are live.
                                    if row == 29 && start_x == 31 {
                                        let saved =
                                            (cpu.a, cpu.f, cpu.bc(), cpu.de(), cpu.hl(), cpu.sp);
                                        let saved_cache = cache(&bus);
                                        cpu.sp -= 2;
                                        bus.write(cpu.sp, 7);
                                        bus.write(cpu.sp + 1, 0);
                                        cpu.pc = 0x38;
                                        ...
                                        assert_eq!(cache(&bus), saved_cache);
                                        assert_eq!(bus.read(0xcb04), ticks.wrapping_add(1));
                                        assert_eq!(bus.read(0xcb03), 0xa1);
                                        assert_eq!(
                                            bus.read(0xc8f9),
                                            0,
                                            "guest NMI must not execute"
                                        );
                                        interrupts += 1;
                                    }
                                }
                                let saved_cache = cache(&bus);
                                let (cpu, ticks) = segment(
                                    &mut bus,
                                    "_m3r_cell_source",
                                    defs["_m3r_attribute_ready"].1,
                                );
                                max_primary = max_primary.max(ticks);
                                let base = if record == 0 { 0x800 } else { 0x1000 };
                                let tile =
                                    bus.cart_ram[base + usize::from(mirrored(v, horizontal))];
                                let map_index =
                                    u16::from((ctrl & 0x10) >> 2) | u16::from(tile >> 6);
                                let page = bus.cart_ram[0x1900 + usize::from(record + map_index)];
                                let expected_key = u16::from(page) * 64 + u16::from(tile & 63);
                                assert_eq!(
                                    u16::from_le_bytes([bus.ram[0x84b], bus.ram[0x84c]]),
                                    expected_key
                                );
                                let attr_v =
                                    (v & 0xc00) | 0x3c0 | ((v >> 4) & 0x38) | ((v >> 2) & 7);
                                assert_eq!(
                                    cpu.a,
                                    bus.cart_ram[base + usize::from(mirrored(attr_v, horizontal))]
                                );
                                assert_eq!(cpu.c, v as u8 & 0x42);
                                assert_eq!(cache(&bus), saved_cache);
                                assert_eq!(bus.read(0xc849), record as u8);
                                assert_eq!(u16::from_le_bytes([bus.ram[0x846], bus.ram[0x847]]), v);
                                assert_eq!((bus.slot_bank, bus.read(0xfffc)), mapping);
                                cells += 1;
                                v = if v & 31 == 31 {
                                    (v & !31) ^ 0x400
        ...
        assert_eq!(&bus.ram[..0x800], guest);
        assert_eq!(&bus.ram[0x1300..0x1600], protected);
        assert_eq!(cells, 81_920);
        assert_eq!(interrupts, 32);
        assert_eq!(
            max_setup, 602,
            "nominal setup including RET, excluding caller CALL/EI/NOP"
        );
        eprintln!(
            "MMC3 row address kernel: {cells} source-derived cells, {interrupts} real host IRQ boundaries; max setup{max_setup} nominalT, primary-to-raw-attribute{max_primary} nominalT"
        );
    }

    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT and TRACE_ROW_BASELINE_PROJECT pre-kernel renderer"]
    fn mmc3_full_row_kernel_preserves_mixed_source_fallback() {
        let project = |name| {
        ...
        let old = project("TRACE_ROW_BASELINE_PROJECT");
        let new = project("TRACE_FUNCTIONAL_PROJECT");
        let projects = [old, new].map(|path| {
            (
                load_wla_symbol_defs(&path.join("sms.sym")),
                std::fs::read(path.join("sms.sms")).unwrap(),
            )
        });
        assert!(!projects[0].0.contains_key("_m3r_row_addresses"));
        assert!(projects[1].0.contains_key("_m3r_row_addresses"));
        let mut cases = 0;
        let mut rejected = 0;
        let mut secondary = 0;
        let mut max_closed = [0u64; 2];
        let mut new_interrupts = 0;
        for record in [0u8, 0x40] {
            for mirror in [0u8, 1] {
                for fine in 0..8u8 {
                    for split in [0u8, 192, 193] {
                        for reload in [0u8, 1] {
                            for coarse_y in [0u16, 29, 31] {
                                for col in [0u8, 31] {
                                    let mut outcomes = Vec::new();
                                    for (variant, (defs, rom)) in projects.iter().enumerate() {
                                        let mut bus = SmsBus::new(rom.clone(), 0xff);
                                        bus.write(0xfffc, 8);
                                        bus.write(0xfffe, 17);
                                        bus.write(0xcb14, 17);
                                        ...
                                        for offset in 0..0x1000u16 {
                                            bus.write(
                                                0x8800 + offset,
                                                (offset as u8).wrapping_mul(37)
                                                    ^ (offset >> 7) as u8,
                                            );
                                        }
                                        for rec in [0u16, 0x40] {
                                            for index in 0..8u16 {
                                                bus.write(
                                                    0x9900 + rec + index,
                                                    ((index * 19 + rec + 5) & 255) as u8,
                                                );
                                            }
                                            bus.write(
                                                0x9908 + rec,
                                                if rec == 0 { 0x08 } else { 0x18 },
                                            );
                                            bus.write(0x990f + rec, mirror ^ u8::from(rec != 0));
                                        }
                                        let v = 0x800 | (coarse_y << 5) | u16::from(col);
                                        bus.write(0xc846, v as u8);
                                        bus.write(0xc847, (v >> 8) as u8);
                                        bus.write(0xc849, record);
                                        bus.write(0xc842, 24);
                                        bus.write(0xc843, col);
                                        bus.write(0xc848, fine);
                                        bus.write(0xc837, split);
                                        bus.write(0xc8ea, if record == 0 { 0 } else { fine });
                                        bus.write(0xc8eb, 0);
                                        bus.write(0x994c, 1);
                                        bus.write(0x994d, 4);
                                        let hud_v = (u16::from(7 - fine) << 12) | (coarse_y << 5);
                                        bus.write(0x9970, hud_v as u8);
                                        bus.write(0x9971, (hud_v >> 8) as u8);
                                        bus.write(0x9972, reload);
                                        let frozen = bus.cart_ram;
                                        let guest = bus.ram[..0x800].to_vec();
                                        let protected = bus.ram[0x1300..0x1600].to_vec();
                                        let mapping = (bus.slot_bank, bus.read(0xfffc));
                                        let mut cpu = Cpu::new();
                                        cpu.sp = 0xdff0;
                                        if variant == 1 {
                                            cpu.pc = defs["_m3r_row_addresses"].1;
                                            bus.write(cpu.sp, 7);
                                            bus.write(cpu.sp + 1, 0);
                                            for _ in 0..500 {
                                                if cpu.pc == 7 {
                                        ...
                                        cpu.pc = defs["_m3r_cell"].1;
                                        let mut closed = 0;
                                        let mut injected = false;
                                        let mut visited_secondary = false;
                                        for _ in 0..4000 {
                                            if cpu.pc == defs["_m3r_bg_ready"].1
                                                || cpu.pc == defs["rt_mmc3_raster_unsupported"].1
                                            {
                                                break;
                                            }
                                            visited_secondary |= cpu.pc
                                                == defs["_m3r_cell_source"].1
                                                && bus.ram[0x8e9] != 0;
                                            if cpu.iff1 && cpu.ei_pending == 0 {
                                                max_closed[variant] =
                                                    max_closed[variant].max(closed);
                                                closed = 0;
                                                if !injected {
                                                    let pc = cpu.pc;
                                                    let context = (
                                                    ...
                                                    cpu.sp -= 2;
                                                    bus.write(cpu.sp, pc as u8);
                                                    bus.write(cpu.sp + 1, (pc >> 8) as u8);
                                                    cpu.pc = 0x38;
                                                    cpu.iff1 = false;
                                                    cpu.iff2 = false;
                                                    bus.frame_int_pending = true;
                                            ...
                                            closed += mmc3_row_nominal(&cpu, &mut bus);
                                            cpu.step(&mut bus).unwrap();
                                            assert_eq!(bus.read(0xcb1d), 0);
                                        }
                                        let trapped =
                                            cpu.pc == defs["rt_mmc3_raster_unsupported"].1;
                                        assert!(trapped || cpu.pc == defs["_m3r_bg_ready"].1);
                                        // Successful cells next execute EI/NOP before lookup.
                                        if !trapped {
                                            max_closed[variant] =
                                                max_closed[variant].max(closed + 8);
                                        }
                                        if !trapped {
                                            assert_eq!(cpu.sp, 0xdff0);
                                        }
                                        assert_eq!(&bus.cart_ram, &frozen);
                                        assert_eq!(&bus.ram[..0x800], guest);
                                        assert_eq!(&bus.ram[0x1300..0x1600], protected);
                                        assert_eq!((bus.slot_bank, bus.read(0xfffc)), mapping);
                                        assert_eq!(bus.read(0xcb03), 0xa1);
                                        ...
                                        if variant == 1 {
                                            assert_eq!(
                                                ...
                                                cache,
                                                "secondary must not poison primary cache"
                                            );
                                            new_interrupts += usize::from(injected);
                                        }
                                        let result: Vec<_> = [
                                            0x846, 0x847, 0x849, 0x84a, 0x84b, 0x84c, 0x8e4, 0x8e5,
                                            0x8e6, 0x8e7, 0x8ea, 0x8eb,
                                        ]
                                        .map(|offset| bus.ram[offset])
                                        .into();
                                        outcomes.push((trapped, visited_secondary, result));
                                    }
                                    assert_eq!(
                                        outcomes[0], outcomes[1],
                                        "record{record:02X} mirror{mirror} fine{fine} split{split} reload{reload} y{coarse_y} col{col}"
                                    );
                                    rejected += usize::from(outcomes[0].0);
                                    secondary += usize::from(outcomes[0].1);
                                    cases += 1;
                                }
                            }
                        }
        ...
        assert!(rejected > 0 && secondary > 0 && new_interrupts > 0);
        eprintln!(
            "row source old/new: {cases} cases, {secondary} secondary paths, {rejected} matching strict three-source rejections, {new_interrupts} new-kernel host IRQs; closed spans old/new nominalT{max_closed:?} (excluding interrupt service)"
        );
    }
    ...
    fn mmc3_full_needed_lifetime_matches_all_committed_rows() {
        ...
        for fine_y in [0, 7] {
            ...
            for (tile, stable, mask) in [
                ...
                for page in 0..2u16 {
                    for offset in 0..0x3c0 {
                        mmc3_test_ciram_store(
                            &mut bus,
                            defs["rt_ppu_write"].1,
                            0x2000 + page * 0x800 + offset,
                            tile,
                        );
                    }
                }
    ...
    #[test]
    #[ignore = "requires TRACE_FUNCTIONAL_PROJECT from mmc3_full_irq_bridge_fixture"]
    fn mmc3_full_irq_bridge_preserves_context_and_pending_order() {
        let path = PathBuf::from(std::env::var("TRACE_FUNCTIONAL_PROJECT").unwrap());
        let path = if path.is_absolute() {
            path
        } else {
        ...
        for control in [0, 8, 12] {
            ...
            for (index, page) in [0, 1, 6, 7].into_iter().enumerate() {
                bus.write(0xc819 + index as u16, page);
            }
            let invoke = |bus: &mut SmsBus, nested: bool, request: bool, trap: bool| {
                let original_mapping = (bus.slot_bank, bus.read(0xfffc));
                let original_p = bus.read(0xcb03);
                let mut cpu = Cpu::new();
                cpu.pc = defs["rt_mmc3_sms_interrupt"].1;
                cpu.sp = 0xdff0;
                cpu.a = 0x6d;
                cpu.f = 0x95;
                cpu.set_bc(0x8743);
                ...
                bus.write(cpu.sp + 1, 0);
                bus.frame_int_pending = true;
                bus.write(0xc825, u8::from(request));
                let mut nested_fired = false;
                for _ in 0..3_000_000 {
                    if cpu.pc == 7 || bus.read(0xcb1d) != 0 {
                        break;
                    }
                    if nested && !nested_fired && cpu.iff1 && bus.read(0xc831) == 1 {
                        // Actual nested IM1 context at a guest-handler safe
                        // boundary: host-service testing, not an A12 clock.
                ...
                if trap {
                    assert_eq!(bus.read(0xcb1d), 0xea);
                    assert_eq!(
                        bus.read(0xc823),
                        1,
                        "stale IRQ must not be silently acknowledged"
                    );
                    return;
                }
                assert_eq!(bus.read(0xcb1d), 0);
                ...
                assert_eq!(
                    (cpu.a, cpu.f, cpu.bc(), cpu.de(), cpu.hl(), cpu.sp),
                    (0x6d, 0x95, 0x8743, 0x52a9, 0x67de, 0xdff2)
                );
                assert_eq!((bus.slot_bank, bus.read(0xfffc)), original_mapping);
                assert_eq!(bus.read(0xcb14), 17);
                assert_eq!(bus.read(0xcb03), original_p);
                assert_eq!(bus.read(0xcb02), 0xfd);
                assert_eq!(
                    [bus.read(0xcb15), bus.read(0xcb18), bus.read(0xcb27)],
                    [0x23, 0x45, 0x67]
                );
                assert_eq!(bus.read(0xc831), 0);
                assert_eq!(bus.read(0xca11), 0);
                assert!(cpu.iff1 && cpu.iff2);
                assert_eq!(nested_fired, nested);
                assert_eq!(bus.read(0xc825), 0, "request must be consumed exactly once");
            };
            let write_mask = |bus: &mut SmsBus, mask: u8| {
                let mut cpu = Cpu::new();
                cpu.pc = defs["rt_ppu_write"].1;
                cpu.sp = 0xdff0;
                cpu.a = mask;
            ...
            invoke(&mut bus, true, true, false);
            assert_eq!((bus.read(0xc070), bus.read(0xc071)), (1, 0));
            assert_eq!(bus.read(0xc832), 1);
            invoke(&mut bus, false, false, false); // physical host service only
            assert_eq!((bus.read(0xc070), bus.read(0xc071)), (1, 0));
            bus.write(0xc072, 1); // actual NMI handler acknowledges old pending
            bus.write(0xcb03, 0xa5);
            invoke(&mut bus, true, true, false);
            assert_eq!((bus.read(0xc070), bus.read(0xc071)), (2, 0));
            assert_eq!((bus.read(0xc823), bus.read(0xc8f8)), (0, 0));
            bus.write(0xc072, 0);
            bus.write(0xcb03, 0xa1);
            invoke(&mut bus, true, true, false);
            assert_eq!((bus.read(0xc070), bus.read(0xc071)), (3, 1));
            assert_eq!(bus.read(0xc823), 0); // translated IRQ acknowledged E000
            assert_eq!(bus.read(0xc832), 1); // next translated NMI armed split
            // Main briefly blanks then re-enables before its next wait. The
            // not-yet-asserted raster event is canceled, not delivered using
            // a bank context from half-completed main work.
            write_mask(&mut bus, 0);
            write_mask(&mut bus, 0x18);
            assert_eq!(bus.read(0xc8f6), 1);
            invoke(&mut bus, true, true, false);
            assert_eq!((bus.read(0xc070), bus.read(0xc071)), (4, 1));
            assert_eq!(bus.read(0xc8f6), 0);
            bus.write(0xc831, 1); // transient guest-handler writes do not cancel
            write_mask(&mut bus, 0);
            write_mask(&mut bus, 0x18);
            assert_eq!(bus.read(0xc8f6), 0);
            bus.write(0xc831, 0);
            bus.write(0xcb08, 0x28); // current NMI=0, no sticky legacy policy
            invoke(&mut bus, true, true, false);
            assert_eq!((bus.read(0xc070), bus.read(0xc071)), (4, 2));
            assert_eq!(bus.read(0xc832), 0);
            write_mask(&mut bus, 0);
            invoke(&mut bus, false, true, false);
            assert_eq!((bus.read(0xc070), bus.read(0xc071)), (4, 2));
            assert_eq!(
                bus.read(0xc833),
                0,
                "completed main off epoch must blank with NMI off"
            );
            assert_eq!(bus.vdp_regs[1] & 0x40, 0);
            write_mask(&mut bus, 0x18);
            bus.write(0xcb08, 0xa8);
            invoke(&mut bus, true, true, false);
            assert_eq!((bus.read(0xc070), bus.read(0xc071)), (5, 2));
            bus.write(0xcb03, 0xa5);
            invoke(&mut bus, true, true, false);
            assert_eq!((bus.read(0xc070), bus.read(0xc071)), (6, 2));
            assert_eq!((bus.read(0xc823), bus.read(0xc8f8)), (1, 1));
            write_mask(&mut bus, 0);
            write_mask(&mut bus, 0x18);
            assert_eq!(
                bus.read(0xc823),
                1,
                "main blanking cannot acknowledge pending IRQ"
            );
            bus.write(0xcb03, 0xa1);
            invoke(&mut bus, false, true, true);
            assert_eq!((bus.read(0xc070), bus.read(0xc071)), (6, 2));
        }
    }
}
